...
brew install Clearlove7Zz/tap/loongcode # macOS og Linux (anbefalet, altid up to date)
brew install loongcode              # macOS og Linux (officiel brew formula, opdateres sjældnere)
sudo pacman -S loongcode            # Arch Linux (Stable)
paru -S loongcode-bin               # Arch Linux (Latest from AUR)
mise use -g loongcode               # alle OS
nix run nixpkgs#loongcode           # eller github:Clearlove7Zz/LoongCode for nyeste dev-branch
```
...
Loongcode findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/Clearlove7Zz/LoongCode/releases) eller [modelhub.lgdg.cc/download](https://modelhub.lgdg.cc/download).
...
1. `$LOONGCODE_INSTALL_DIR` - Tilpasset installationsmappe
...
Loongcode har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
For mere info om konfiguration af Loongcode, [**se vores docs**](https://modelhub.lgdg.cc/docs).

### Bidrag

Hvis du vil bidrage til Loongcode, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på Loongcode

Hvis du arbejder på et projekt der er relateret til Loongcode og bruger "loongcode" som en del af navnet; f.eks. "loongcode-dashboard" eller "loongcode-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af Loongcode-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/loongcode) | [X.com](https://x.com/loongcode)
