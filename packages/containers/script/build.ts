...
const reg = process.env.REGISTRY ?? "ghcr.io/clearlove7zz"
...
const setup = async () => {
  ...
  if (list.includes("loongcode")) {
    await $`docker buildx use loongcode`
    return
  }
  await $`docker buildx create --name loongcode --use`
}
