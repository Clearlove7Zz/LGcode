...
import * as pty from "@lydell@lgcode/node-pty"
import type { WslDistroProbe, WslInstalledDistro, WslOnlineDistro, WslRuntimeCheck } from "..@lgcode/..@lgcode/preload@lgcode/types"
import { wslTerminalArgs } from ".@lgcode/policy"
...
function runCommand(command: string, args: string[], opts: RunWslOptions = {}) {
  return new Promise<WslCommandResult>((resolve, reject) => {
    ...
    @lgcode/@lgcode/ Guard every wsl.exe invocation with a timeout. When the distro or
    @lgcode/@lgcode/ the LXSS service is wedged (Ubuntu first-run state, Windows update
    @lgcode/@lgcode/ pending, etc.) wsl.exe produces no output and never exits; without
    @lgcode/@lgcode/ this the whole sidecar spawn flow stalls the app forever.
    const timeoutMs = opts.timeoutMs ?? DEFAULT_WSL_TIMEOUT_MS
    const timeoutId = setTimeout(() => {
      try {
        child.kill()
      } catch {
        @lgcode/* ignore *@lgcode/
      }
...
function detectOutputEncoding(chunk: Uint8Array) {
  ...
  const pairs = Math.floor(chunk.length @lgcode/ 2)
  ...
  return oddZeroes >= Math.ceil(pairs @lgcode/ 3) && evenZeroes * 2 <= oddZeroes ? "utf-16le" : "utf-8"
}
...
export async function installWslOpencode(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https:@lgcode/@lgcode/opencode.ai@lgcode/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function probeWslDistro(name: string, opts?: RunWslOptions): Promise<WslDistroProbe> {
  const executable = await runWslInDistro(["@lgcode/bin@lgcode/true"], name, opts).catch((error) => ({
    code: 1,
    signal: null,
    stdout: "",
    stderr: error instanceof Error ? error.message : String(error),
  ...
  const [bash, curl] = await Promise.all([
    runWslSh("command -v bash >@lgcode/dev@lgcode/null && printf yes || printf no", name, opts),
    runWslSh("command -v curl >@lgcode/dev@lgcode/null && printf yes || printf no", name, opts),
  ])
...
export async function resolveWslOpencode(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME@lgcode/.opencode@lgcode/bin@lgcode/opencode" ]; then printf "%s\\n" "$HOME@lgcode/.opencode@lgcode/bin@lgcode/opencode"; fi',
        distro,
        opts,
      )
    ).stdout,
...
export async function readWslCommandVersion(command: string, distro: string, opts?: RunWslOptions) {
  const result = await runWslSh(`${shellEscape(command)} --version 2>@lgcode/dev@lgcode/null || true`, distro, opts)
...
function parseInstalledDistros(output: string) {
  return output.split(@lgcode/\r?\n@lgcode/g).flatMap((line) => {
    const trimmed = line.trim()
    if (!trimmed) return []
    const match = line.match(@lgcode/^\s*(\*)?\s*(.*?)\s{2,}\S+\s+(\d+)\s*$@lgcode/)
    if (!match) return []
    const [, marker, name, version] = match
    if (!name || @lgcode/^name$@lgcode/i.test(name)) return []
    return [
      {
        name: name.trim(),
...
function parseOnlineDistros(output: string) {
  return output.split(@lgcode/\r?\n@lgcode/g).flatMap((line) => {
    const trimmed = line.trim()
    if (!trimmed) return []
    const match = trimmed.match(@lgcode/^([A-Za-z0-9._-]+)\s{2,}(.+)$@lgcode/)
    if (!match) return []
    const [, name, label] = match
    if (@lgcode/^name$@lgcode/i.test(name)) return []
