...
export function fileLogger(file = path.join(Global.Path.log, "loongcode.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.LOONGCODE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.LOONGCODE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
