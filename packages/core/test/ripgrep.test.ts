    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".gitignore"), "node_modules@lgcode/\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make("src@lgcode/index.js"))
          expect(files.map((item) => item.path)).not.toContain(RelativePath.make("node_modules@lgcode/pkg@lgcode/index.js"))
        }),
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          const files = yield* ripgrep.find({ cwd: tmp.path, pattern: "**@lgcode/*", limit: 10 })
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".opencode@lgcode/config"))
          expect(files.map((item) => item.path)).not.toContain(RelativePath.make(".git@lgcode/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".opencode@lgcode/config"))
          expect(matches.map((item) => item.entry.path)).not.toContain(RelativePath.make(".git@lgcode/config"))
        }),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ),
  )
