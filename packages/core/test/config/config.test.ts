...
import { ConfigProvider } from "@lgcode/core/config/provider"
import { ConfigMigrateV1 } from "@lgcode/core/v1/config/migrate"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "lgcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "lgcode.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "lgcode.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .lgcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".lgcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".lgcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "lgcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "lgcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "lgcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "lgcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".lgcode", "lgcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".lgcode", "lgcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".lgcode")),
              AbsolutePath.make(path.join(directory, ".lgcode")),
            ])
