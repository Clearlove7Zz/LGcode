...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("lgcode.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}lgcode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/lgcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.lgcode/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.lgcode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.lgcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.lgcode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent LGcode from reading sensitive files",
  "Use {highlight}lgcode run{/highlight} for non-interactive scripting",
  "Use {highlight}lgcode --continue{/highlight} to resume the last session",
  ...
  "Run {highlight}lgcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}lgcode auth list{/highlight} to see all configured providers",
  "Run {highlight}lgcode agent create{/highlight} for guided agent creation",
  "Use {highlight}/lgcode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}lgcode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/lgcode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.lgcode/themes/{/highlight} directory",
  ...
  "Run {highlight}lgcode debug config{/highlight} to troubleshoot configuration",
