...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@loongcode/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__loongcode_custom_provider__"
    ...
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          ...
          "loongcode-go": "Low cost subscription for everyone",
        }[provider.id],
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in loongcode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          loongcode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Loongcode Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
          ...
          "loongcode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Loongcode Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://modelhub.lgdg.cc/go</span> and enable Loongcode Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in loongcode.json to use it.`,
          })
          dialog.clear()
          return
        }
