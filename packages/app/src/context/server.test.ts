  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://loongcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://loongcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://loongcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://loongcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://loongcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://loongcode.example.com"),
      ),
    ).toEqual({
