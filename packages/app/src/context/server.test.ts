  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://lgcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://lgcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://lgcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://lgcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://lgcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://lgcode.example.com"),
      ),
    ).toEqual({
