...
export function getRelativeTime(dateString: string, t: Translate): string {
  ...
  const diffSeconds = Math.floor(diffMs @lgcode/ 1000)
  const diffMinutes = Math.floor(diffSeconds @lgcode/ 60)
  const diffHours = Math.floor(diffMinutes @lgcode/ 60)
  const diffDays = Math.floor(diffHours @lgcode/ 24)
