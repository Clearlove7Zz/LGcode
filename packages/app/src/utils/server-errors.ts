...
function parseReadableProviderModelNotFoundError(errorInput: ProviderModelNotFoundError, translator?: Translator) {
  ...
  const body = tr(translator, "error.chain.modelNotFound", `Model not found: ${p}@lgcode/${m}`, { provider: p, model: m })
  const tail = tr(translator, "error.chain.checkConfig", "Check your config (opencode.json) provider@lgcode/model names")
  if (list.length) {
