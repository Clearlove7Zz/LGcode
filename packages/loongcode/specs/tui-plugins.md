...
- Author package entrypoint is `@loongcode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://modelhub.lgdg.cc/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/loongcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@loongcode-ai/plugin/tui`.
- `@loongcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@loongcode-ai/plugin/tui"
...
- The value is a semver range checked against the running Loongcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.loongcode` is absent, no check is performed (backward compatible).
...
- `loongcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `loongcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.loongcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.loongcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.loongcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.loongcode` dir or `LOONGCODE_CONFIG_DIR`, Loongcode installs `@loongcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@loongcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Loongcode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Loongcode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Loongcode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@loongcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"loongcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `loongcode.default`.
...
- Local plugins persist installed themes under the local `.loongcode/themes` area near the plugin config source.
...
- `--pure` / `LOONGCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.loongcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.loongcode/plugins/tui-vim.tsx`
- Local smoke config: `.loongcode/tui.json`
- Local smoke theme: `.loongcode/plugins/smoke-theme.json`
