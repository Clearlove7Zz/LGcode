...
Trim `packages/loongcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/loongcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/loongcode`.
- `[x]` `bun typecheck` from `packages/loongcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/loongcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/loongcode`.
- `[x]` `bun dev generate > /tmp/loongcode-openapi.json` from `packages/loongcode`.
...
- Edit `packages/loongcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/loongcode-openapi.json` from `packages/loongcode`.
...
- Check source schemas in files like `packages/loongcode/src/session/schema.ts`, `packages/loongcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/loongcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/loongcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
