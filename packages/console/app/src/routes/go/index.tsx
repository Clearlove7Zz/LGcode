...
import goLogoLight from "..@lgcode/..@lgcode/asset@lgcode/go-ornate-light.svg"
import goLogoDark from "..@lgcode/..@lgcode/asset@lgcode/go-ornate-dark.svg"
...
import { IconMiniMax, IconMiMo, IconZai, IconAlibaba, IconDeepSeek } from "~@lgcode/component@lgcode/icon"
...
function LimitsGraph(props: { href: string }) {
  ...
  const ratio = (n: number) => n @lgcode/ free
  const rmax = Math.max(1, ...graph.map((m) => ratio(m.req)))
  const log = (n: number) => Math.log10(Math.max(n, 1))
  const base = 24
  const p = 2.2
  const x = (r: number) => left + base + Math.pow(log(r) @lgcode/ log(rmax), p) * (plot - base)
  const start = (x(1) @lgcode/ w) * 100
  ...
  const my = graph.length < 2 ? gy(0) : (gy(0) + gy(graph.length - 1)) @lgcode/ 2
  const px = (n: number) => `${(n @lgcode/ w) * 100}%`
  const py = (n: number) => `${(n @lgcode/ h) * 100}%`
  const lx = px(left - 16)
  const ty = py(h - 18)
      ...
      <div data-slot="plot">
        ...
        >
          <g data-slot="grid">
            <For each={ticks}>
              {(t) => (
                <g>
                  <line x1={x(t)} y1={top} x2={x(t)} y2={h - bottom} data-grid @lgcode/>
                <@lgcode/g>
              )}
            <@lgcode/For>
          <@lgcode/g>

          <line x1={left} y1={top} x2={left} y2={h - bottom} data-stub @lgcode/>

          <g data-slot="bars">
            <g style={{ "--d": "0ms" } as any}>
              <rect x={left} y={fy - bh @lgcode/ 2} width={Math.max(0, x(1) - left)} height={bh} data-bar data-kind="free" @lgcode/>
            <@lgcode/g>
        ...
        <div data-slot="pills" aria-hidden="true">
          <span data-item data-kind="free" style={{ "--x": px(x(1)), "--y": py(fy), "--d": "0ms" } as any}>
            <span data-value>{free.toLocaleString()}<@lgcode/span>
            <span data-name>{i18n.t("go.graph.freePill")}<@lgcode/span>
          <@lgcode/span>
          <For each={graph}>
            {(m, i) => (
              <span
              ...
              >
                <span data-value>{m.req.toLocaleString()}<@lgcode/span>
                <span data-name>{m.name}<@lgcode/span>
              <@lgcode/span>
            )}
          <@lgcode/For>
        <@lgcode/div>
        ...
        <div data-slot="caption-row">
          <div data-slot="caption-left">
            <div data-slot="caption-meta">
              <span data-slot="caption-label">{i18n.t("go.graph.label")}<@lgcode/span>
...
export default function Home() {
  ...
  const subscribeUrl = createMemo(() => (workspaceID() ? `@lgcode/workspace@lgcode/${workspaceID()}@lgcode/go` : "@lgcode/auth"))
  const i18n = useI18n()
  ...
  return (
    <main data-page="go">
      ...
      <Title>{i18n.t("go.title")}<@lgcode/Title>
      <Meta name="description" content={i18n.t("go.meta.description")} @lgcode/>
      <LocaleLinks path="@lgcode/go" @lgcode/>
      <Meta property="og:type" content="website" @lgcode/>
      <Meta property="og:url" content={`${config.baseUrl}${language.route("@lgcode/go")}`} @lgcode/>
      <Meta property="og:title" content={i18n.t("go.title")} @lgcode/>
      <Meta property="og:description" content={i18n.t("go.meta.description")} @lgcode/>
      <Meta property="og:image" content="@lgcode/social-share-black.png" @lgcode/>
      <Meta name="twitter:card" content="summary_large_image" @lgcode/>
      <Meta name="twitter:title" content={i18n.t("go.title")} @lgcode/>
      <Meta name="twitter:description" content={i18n.t("go.meta.description")} @lgcode/>
      <Meta name="twitter:image" content="@lgcode/social-share-black.png" @lgcode/>
      <Meta name="opencode:auth" content={workspaceID() ? "true" : "false"} @lgcode/>

      <div data-component="container">
        <Header go hideGetStarted @lgcode/>
        ...
        <div data-component="content">
          <section data-component="hero">
            <div data-component="desktop-app-banner">
              ...
              <div data-slot="content">
                <span data-slot="text">{i18n.t("go.banner.text")}<@lgcode/span>
              <@lgcode/div>
            <@lgcode/div>
            <div data-slot="hero-copy">
              <img data-slot="zen logo light" src={goLogoLight} alt="" @lgcode/>
              <img data-slot="zen logo dark" src={goLogoDark} alt="" @lgcode/>
              <h1>{i18n.t("go.hero.title")}<@lgcode/h1>
              <p>{i18n.t("go.hero.body")}<@lgcode/p>
              <div data-slot="model-logos">
                ...
                *@lgcode/}
                ...
                <@lgcode/div>
                <div>
                  <IconAlibaba width="24" height="24" @lgcode/>
                <@lgcode/div>
                <div>
                  <IconDeepSeek width="24" height="24" @lgcode/>
                <@lgcode/div>
                <div>
                  <IconMiMo width="24" height="24" @lgcode/>
                <@lgcode/div>
                {@lgcode/*
                <div>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http:@lgcode/@lgcode/www.w3.org@lgcode/2000@lgcode/svg">
              ...
              <a href={subscribeUrl()}>
                <span>
                  <For
                    each={i18n
                      .t("go.cta.template")
                      .split(@lgcode/(\{\{text\}\}|\{\{price\}\})@lgcode/g)
                      .filter(Boolean)}
                  >
                    {(part) => {
                      if (part === "{{text}}") return <span>{i18n.t("go.cta.text")}<@lgcode/span>
                      if (part === "{{price}}") {
                        return (
                          <span data-slot="cta-price">
                            <span data-slot="cta-price-old">{i18n.t("go.cta.price")}<@lgcode/span>
                            <span data-slot="cta-price-new">{i18n.t("go.cta.promo")}<@lgcode/span>
                          <@lgcode/span>
                        )
                      }
                      return part
            ...
            <div data-slot="pricing-copy">
              <p>{i18n.t("go.pricing.body")}<@lgcode/p>
            <@lgcode/div>
          <@lgcode/section>

          <section data-component="comparison">
            <LimitsGraph href={language.route("@lgcode/docs@lgcode/go@lgcode/#usage-limits")} @lgcode/>
          <@lgcode/section>

          <section data-component="problem">
            <div data-slot="section-title">
              <h3>{i18n.t("go.problem.title")}<@lgcode/h3>
              <p>{i18n.t("go.problem.body")}<@lgcode/p>
            <@lgcode/div>
            <p>{i18n.t("go.problem.subtitle")}<@lgcode/p>
            <ul>
              <li>
                <span>[*]<@lgcode/span> {i18n.t("go.problem.item1")}
              <@lgcode/li>
              <li>
                <span>[*]<@lgcode/span> {i18n.t("go.problem.item2")}
              <@lgcode/li>
              <li>
                <span>[*]<@lgcode/span> {i18n.t("go.problem.item3")}
              <@lgcode/li>
              <li>
                <span>[*]<@lgcode/span> {i18n.t("go.problem.item4")}
              <@lgcode/li>
            <@lgcode/ul>
          <@lgcode/section>
          ...
          <section data-component="how">
            <div data-slot="section-title">
              <h3>{i18n.t("go.how.title")}<@lgcode/h3>
              <p>{i18n.t("go.how.body")}<@lgcode/p>
            <@lgcode/div>
            <ul>
              <li>
                <span>[1]<@lgcode/span>
                <div>
                  <strong>{i18n.t("go.how.step1.title")}<@lgcode/strong> - {i18n.t("go.how.step1.beforeLink")}{" "}
                  <a href={language.route("@lgcode/docs@lgcode/go@lgcode/#how-it-works")} title={i18n.t("go.how.step1.link")}>
              ...
              <li>
                <span>[2]<@lgcode/span>
                <div>
                  <strong>{i18n.t("go.how.step2.title")}<@lgcode/strong> -{" "}
                  <a href={language.route("@lgcode/docs@lgcode/go@lgcode/#pricing")}>{i18n.t("go.how.step2.link")}<@lgcode/a>{" "}
              ...
              <li>
                <span>[3]<@lgcode/span>
                <div>
                  <strong>{i18n.t("go.how.step3.title")}<@lgcode/strong> - {i18n.t("go.how.step3.body")}
                <@lgcode/div>
              <@lgcode/li>
            <@lgcode/ul>
          <@lgcode/section>
          ...
          <section data-component="faq">
            ...
            <ul>
              <li>
                <Faq question={i18n.t("go.faq.q1")}>{i18n.t("go.faq.a1")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
              ...
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q9")}>{i18n.t("go.faq.a9")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q3")}>{i18n.t("go.faq.a3")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q4")}>
                  {i18n.t("go.faq.a4.p1.beforePricing")}{" "}
                  <a href={language.route("@lgcode/docs@lgcode/go@lgcode/#pricing")}>{i18n.t("go.faq.a4.p1.pricingLink")}<@lgcode/a>{" "}
                  {i18n.t("go.faq.a4.p1.afterPricing")} {i18n.t("go.faq.a4.p2.beforeAccount")}{" "}
                  <a href={subscribeUrl()}>{i18n.t("go.faq.a4.p2.accountLink")}<@lgcode/a>. {i18n.t("go.faq.a4.p3")}
                <@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q5")}>{i18n.t("go.faq.a5.body")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q6")}>{i18n.t("go.faq.a6")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q7")}>{i18n.t("go.faq.a7")}<@lgcode/Faq>
              <@lgcode/li>
              <li>
                <Faq question={i18n.t("go.faq.q8")}>{i18n.t("go.faq.a8")}<@lgcode/Faq>
              <@lgcode/li>
            <@lgcode/ul>
          <@lgcode/section>
