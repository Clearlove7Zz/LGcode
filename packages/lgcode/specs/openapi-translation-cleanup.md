...
Trim `packages/lgcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/lgcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/lgcode`.
- `[x]` `bun typecheck` from `packages/lgcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/lgcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/lgcode`.
- `[x]` `bun dev generate > /tmp/lgcode-openapi.json` from `packages/lgcode`.
...
- Edit `packages/lgcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/lgcode-openapi.json` from `packages/lgcode`.
...
- Check source schemas in files like `packages/lgcode/src/session/schema.ts`, `packages/lgcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/lgcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/lgcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
