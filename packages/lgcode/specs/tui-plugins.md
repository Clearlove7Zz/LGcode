...
- Author package entrypoint is `@lgcode-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/lgcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@lgcode-ai/plugin/tui`.
- `@lgcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@lgcode-ai/plugin/tui"
...
- The value is a semver range checked against the running LGcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.lgcode` is absent, no check is performed (backward compatible).
...
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.lgcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.lgcode` dir or `LGCODE_CONFIG_DIR`, LGcode installs `@lgcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@lgcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus LGcode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
LGcode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, LGcode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@lgcode-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.lgcode/themes` area near the plugin config source.
...
- `--pure` / `LGCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.lgcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.lgcode/plugins/tui-vim.tsx`
- Local smoke config: `.lgcode/tui.json`
- Local smoke theme: `.lgcode/plugins/smoke-theme.json`
