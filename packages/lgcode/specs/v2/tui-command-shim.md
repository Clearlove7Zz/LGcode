...
Delete `packages/loongcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/loongcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/loongcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/loongcode`
- TUI plugin loader tests in `packages/loongcode` if runtime plugin API wiring changed
