...
@lgcode/@lgcode/ @lgcode/config echoes the config back. Padding the config pushes the response body
@lgcode/@lgcode/ well past the 1024 B threshold so we can observe compression behavior.
function fatConfig() {
...
describe("HttpApi compression", () => {
  describe("encodes responses", () => {
    test("gzips JSON when Accept-Encoding includes gzip and body exceeds threshold", async () => {
      ...
      const response = await app().request("@lgcode/config", {
    ...
    test("does not include the original Content-Length when compressed", async () => {
      ...
      @lgcode/@lgcode/ Either absent (transfer-encoding chunked) or matches the compressed length.
  ...
  describe("skips", () => {
    ...
    test("when the response body is below the 1024-byte threshold", async () => {
      @lgcode/@lgcode/ A bare config produces a tiny response (~few hundred bytes).
  ...
  describe("streaming exclusions", () => {
    test("@lgcode/event SSE is not compressed", async () => {
      await using tmp = await tmpdir({ config: { formatter: false, lsp: false } })
      const controller = new AbortController()
      const response = await app().request("@lgcode/event", {
        headers: { "x-opencode-directory": tmp.path, "accept-encoding": "gzip" },
    ...
    test("@lgcode/global@lgcode/event SSE is not compressed", async () => {
      const controller = new AbortController()
      const response = await app().request("@lgcode/global@lgcode/event", {
