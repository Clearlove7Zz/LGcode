...
import type { AppServices } from "@@lgcode/effect@lgcode/app-runtime"
import type { InstanceStore } from "@@lgcode/project@lgcode/instance-store"
import { cmd, type WithDoubleDash } from ".@lgcode/cmd@lgcode/cmd"
 ...
 * formatter in `src@lgcode/cli@lgcode/error.ts` (FormatError), so the existing top-level
 * catch + cleanup in `src@lgcode/index.ts` runs normally.
 *@lgcode/
...
interface EffectCmdOpts<Args, A> {
   ...
   * init + LSP@lgcode/File@lgcode/etc forks) eagerly.
   *
  ...
  @lgcode/** Defaults to process.cwd(). Override for commands that take a directory positional. *@lgcode/
 ...
 *
 * Errors propagate to the existing top-level handler in `src@lgcode/index.ts`; use
 ...
 * `effectCmd`, swapping the underlying `cmd()` factory for effect@lgcode/cli's
...
export const effectCmd = <Args, A>(opts: EffectCmdOpts<Args, A>) =>
  cmd<{}, Args>({
    ...
    async handler(rawArgs) {
      const { AppRuntime } = await import("@@lgcode/effect@lgcode/app-runtime")
      @lgcode/@lgcode/ yargs typing wraps Args in ArgumentsCamelCase<WithDoubleDash<...>>; cast at the boundary.
      ...
      const { InstanceStore } = await import("@@lgcode/project@lgcode/instance-store")
      const { InstanceRef } = await import("@@lgcode/effect@lgcode/instance-ref")
