...
- `../llm.ts` imports `LLMClient` from `@lgcode-ai/llm/route`; native execution is the only path that calls it directly.
...
- `native-request.ts` is the only adapter file that should construct `LLM.request(...)`, `LLM.model(...)`, `Message.*`, `SystemPart`, `ToolCallPart`, `ToolResultPart`, or `ToolDefinition` values from `@lgcode-ai/llm`.
- `ai-sdk.ts` and `native-runtime.ts` both emit `@lgcode-ai/llm` `LLMEvent`s so downstream session processing does not care which runtime handled the request.
...
`native-runtime.ts` evaluates the gate and either bridges into `@lgcode-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays lgcode-owned in both branches; only request lowering and transport differ.

Safety boundary:

- AI SDK remains the default.
- `LGCODE_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `LGCODE_EXPERIMENTAL=true` opts in. Native is not a global replacement.
- Native execution currently supports OpenAI, lgcode-managed OpenAI-compatible, and Anthropic API-key paths backed by `@ai-sdk/openai`, `@ai-sdk/openai-compatible`, or `@ai-sdk/anthropic` catalog entries.
