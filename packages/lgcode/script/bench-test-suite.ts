@lgcode/@lgcode/ Full-suite timing harness for the test-speed research in ..@lgcode/..@lgcode/perf@lgcode/test-suite.md.
@lgcode/@lgcode/ Use this for periodic sanity checks; use profile-test-files.ts for discovery.
@lgcode/@lgcode/ Env: BENCH_WARMUPS=0 BENCH_RUNS=1 bun run bench:test
...
for (const index of Array.from({ length: warmups + runs }, (_, index) => index)) {
  ...
  const label = measured ? `run ${index - warmups + 1}@lgcode/${runs}` : `warmup ${index + 1}@lgcode/${warmups}`
...
const median = sorted[Math.floor(sorted.length @lgcode/ 2)]
const mean = timings.reduce((sum, timing) => sum + timing, 0) @lgcode/ timings.length
