---
title: Server
description: Interact with loongcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `loongcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an loongcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `loongcode.local` |
...
Set `LOONGCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `loongcode`, or set `LOONGCODE_SERVER_USERNAME` to override it. This applies to both `loongcode serve` and `loongcode web`.
...
When you run `loongcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the loongcode server to interact with loongcode programmatically.
:::

This architecture lets loongcode support multiple clients and allows you to interact with loongcode programmatically.

You can run `loongcode serve` to start a standalone server. If you have the
loongcode TUI running, `loongcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Loongcode [IDE](/docs/ide) plugins.
...
The loongcode server exposes the following APIs.
