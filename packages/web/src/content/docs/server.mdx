...
The `lgcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
Set `LGCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `LGCODE_SERVER_USERNAME` to override it. This applies to both `lgcode serve` and `lgcode web`.
...
:::tip
Use the lgcode server to interact with opencode programmatically.
:::
...
opencode TUI running, `lgcode serve` will start a new server.
