---
title: Server
description: Interact with lgcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `lgcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an lgcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `lgcode.local` |
...
Set `LGCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `lgcode`, or set `LGCODE_SERVER_USERNAME` to override it. This applies to both `lgcode serve` and `lgcode web`.
...
When you run `lgcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the lgcode server to interact with lgcode programmatically.
:::

This architecture lets lgcode support multiple clients and allows you to interact with lgcode programmatically.

You can run `lgcode serve` to start a standalone server. If you have the
lgcode TUI running, `lgcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the LGcode [IDE](/docs/ide) plugins.
...
The lgcode server exposes the following APIs.
