...
With LGcode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, LGcode uses our own `lgcode` theme.
...
LGcode comes with several built-in themes.
...
- Want LGcode to match their terminal's appearance
...
LGcode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/lgcode/themes/*.json` or `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.lgcode/themes/*.json`
4. **Current working directory** - Defined in `./.lgcode/themes/*.json`
