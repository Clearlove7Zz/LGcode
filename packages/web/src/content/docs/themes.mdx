...
With Loongcode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Loongcode uses our own `loongcode` theme.
...
Loongcode comes with several built-in themes.
...
- Want Loongcode to match their terminal's appearance
...
Loongcode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/loongcode/themes/*.json` or `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.loongcode/themes/*.json`
4. **Current working directory** - Defined in `./.loongcode/themes/*.json`
