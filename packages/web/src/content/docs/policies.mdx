---
title: Policies
description: Control which configured resources Loongcode may use.
---

Policies control whether Loongcode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `loongcode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Loongcode may use a resource such as an LLM provider.
...
Loongcode currently supports one policy action:
