...
You can add external tools to LGcode using the _Model Context Protocol_, or MCP. LGcode supports both local and remote servers.
...
You can define MCP servers in your [LGcode Config](https://modelhub.lgdg.cc/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/lgcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
LGcode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, LGcode will:
...
If the server requires authentication, LGcode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `lgcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, LGcode will store the tokens securely in `~/.local/share/lgcode/mcp-auth.json`.
...
Your MCPs are available as tools in LGcode, alongside built-in tools. So you can manage them through the LGcode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect LGcode to your Sentry account.
