...
opencode 데스크톱은 배경에서 로컬 opencode 서버 (`lgcode-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.
...
-**macOS/리눅스**: `~/.config/opencode/lgcode.jsonc` (또는 `~/.config/opencode/lgcode.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/lgcode.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\lgcode.jsonc`
