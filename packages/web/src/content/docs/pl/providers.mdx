---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

loongcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji loongcode.
...
w `~/.local/share/loongcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim loongcode
config.
...
Loongcode Zen to lista modeli dostarczonych przez zespół loongcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z loongcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Loongcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Loongcode Zen` i przejdź do [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Działa jak każdy inny dostawca w loongcode i jest całkowicie opcjonalny w użyciu.

---

## Loongcode Go

Loongcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół loongcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z loongcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Loongcode Go` i przejdź do [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Aby używać Amazon Bedrock z loongcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania loongcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `loongcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w loongcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować loongcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod loongcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji loongcode.
...
loongcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować loongcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`loongcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem loongcode:
...
Aby używać Google Vertex AI z loongcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania loongcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą loongcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session), aby automatycznie rejestrować każdą konwersację loongcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja loongcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować loongcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować loongcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować loongcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem loongcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/loongcode).
:::
...
Aby korzystać z Ollama Cloud z loongcode:
...
3. Skopiuj klucz API do użycia w loongcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w loongcode musisz pobrać informacje o modelu lokalnie:
...
Loongcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół loongcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Loongcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Loongcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API loongcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji loongcode.
...
5. Możesz je także dostosować za pomocą konfiguracji loongcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z loongcode:
...
Możesz także dostosować modele za pomocą konfiguracji loongcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z loongcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `loongcode.json` w katalogu projektu:
...
Pola `limit` pozwalają loongcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `loongcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację loongcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji loongcode.
