---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w lgcode.
---

import config from "../../../../config.mjs"
export const console = config.console

lgcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji lgcode.
...
w `~/.local/share/lgcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim lgcode
config.
...
LGcode Zen to lista modeli dostarczonych przez zespół lgcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z lgcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od LGcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `LGcode Zen` i przejdź do [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Działa jak każdy inny dostawca w lgcode i jest całkowicie opcjonalny w użyciu.

---

## LGcode Go

LGcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół lgcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z lgcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `LGcode Go` i przejdź do [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Aby używać Amazon Bedrock z lgcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania lgcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `lgcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w lgcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować lgcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod lgcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji lgcode.
...
lgcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować lgcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`lgcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem lgcode:
...
Aby używać Google Vertex AI z lgcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania lgcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą lgcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session), aby automatycznie rejestrować każdą konwersację lgcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja lgcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować lgcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować lgcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować lgcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem lgcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/lgcode).
:::
...
Aby korzystać z Ollama Cloud z lgcode:
...
3. Skopiuj klucz API do użycia w lgcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w lgcode musisz pobrać informacje o modelu lokalnie:
...
LGcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół lgcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>LGcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **LGcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API lgcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji lgcode.
...
5. Możesz je także dostosować za pomocą konfiguracji lgcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z lgcode:
...
Możesz także dostosować modele za pomocą konfiguracji lgcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z lgcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `lgcode.json` w katalogu projektu:
...
Pola `limit` pozwalają lgcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `lgcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację lgcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji lgcode.
