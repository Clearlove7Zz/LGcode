...
Umiejętności agenta pozwalają lgcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
lgcode przeszukuje te lokalizacje:

- Project config: `.lgcode/skills/<name>/SKILL.md`
- Global config: `~/.config/lgcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu lgcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.lgcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.lgcode/skills/git-release/SKILL.md` w ten sposób:
...
lgcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `lgcode.json`:
...
**Dla agentów wbudowanych** (w `lgcode.json`):
