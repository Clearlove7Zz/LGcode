...
Umiejętności agenta pozwalają loongcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
loongcode przeszukuje te lokalizacje:

- Project config: `.loongcode/skills/<name>/SKILL.md`
- Global config: `~/.config/loongcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu loongcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.loongcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.loongcode/skills/git-release/SKILL.md` w ten sposób:
...
loongcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `loongcode.json`:
...
**Dla agentów wbudowanych** (w `loongcode.json`):
