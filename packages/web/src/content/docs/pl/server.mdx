---
title: Serwer
description: Interakcja z serwerem lgcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `lgcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient lgcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `lgcode.local` |
...
Ustaw `LGCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `lgcode` lub ustaw `LGCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `lgcode serve`, jak i `lgcode web`.
...
Kiedy uruchomisz `lgcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera lgcode do programowej interakcji z lgcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez lgcode i programową interakcję z kodem otwartym.

Możesz uruchomić `lgcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
lgcode TUI działa, `lgcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki lgcode [IDE](/docs/ide).
...
Serwer lgcode udostępnia następujące interfejsy API.
