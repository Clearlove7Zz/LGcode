...
Komenda `lgcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opencode.
...
Ustaw `LGCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `opencode` lub ustaw `LGCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `lgcode serve`, jak i `lgcode web`.
...
opencode TUI działa, `lgcode serve` uruchomi nowy serwer.
