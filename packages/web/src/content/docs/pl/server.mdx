---
title: Serwer
description: Interakcja z serwerem loongcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `loongcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient loongcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `loongcode.local` |
...
Ustaw `LOONGCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `loongcode` lub ustaw `LOONGCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `loongcode serve`, jak i `loongcode web`.
...
Kiedy uruchomisz `loongcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera loongcode do programowej interakcji z loongcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez loongcode i programową interakcję z kodem otwartym.

Możesz uruchomić `loongcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
loongcode TUI działa, `loongcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki loongcode [IDE](/docs/ide).
...
Serwer loongcode udostępnia następujące interfejsy API.
