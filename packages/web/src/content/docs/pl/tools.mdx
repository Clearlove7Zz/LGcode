...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Loongcode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Loongcode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LOONGCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `LOONGCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Loongcode lub gdy zmienna środowiskowa `LOONGCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Loongcode:
