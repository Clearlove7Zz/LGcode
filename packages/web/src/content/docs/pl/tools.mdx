...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. LGcode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w LGcode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LGCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `LGCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy LGcode lub gdy zmienna środowiskowa `LGCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu LGcode:
