...
Możesz dodać dodatkowe informacje do lgcode za pomocą _Model Context Protocol_, w skrócie MCP. lgcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [lgcode Config](https://modelhub.lgdg.cc/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/lgcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
lgcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, lgcode:
...
Jeśli serwer wymaga uwierzytelnienia, lgcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `lgcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji lgcode bezpieczne przechowa tokeny w `~/.local/share/lgcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w lgcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie lgcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć lgcode z kontem Sentry.
