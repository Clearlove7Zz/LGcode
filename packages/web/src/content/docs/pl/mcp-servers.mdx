...
Możesz dodać dodatkowe informacje do loongcode za pomocą _Model Context Protocol_, w skrócie MCP. loongcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [loongcode Config](https://modelhub.lgdg.cc/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/loongcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
loongcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, loongcode:
...
Jeśli serwer wymaga uwierzytelnienia, loongcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `loongcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji loongcode bezpieczne przechowa tokeny w `~/.local/share/loongcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w loongcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie loongcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć loongcode z kontem Sentry.
