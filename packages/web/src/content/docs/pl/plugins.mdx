---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć loongcode.
---

Wtyczki umożliwiające wyłączenie loongcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania loongcode.
...
- `.loongcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/loongcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/loongcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/loongcode/loongcode.json`)
2. Konfiguracja projektu (`loongcode.json`)
3. Globalny katalog wtyczek (`~/.config/loongcode/plugins/`)
4. Katalog wtyczek projektu (`.loongcode/plugins/`)
...
loongcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z loongcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do loongcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej loongcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj loongcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do loongcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod loongcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://modelhub.lgdg.cc/docs/sdk).
