---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć lgcode.
---

Wtyczki umożliwiające wyłączenie lgcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania lgcode.
...
- `.lgcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/lgcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/lgcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/lgcode/lgcode.json`)
2. Konfiguracja projektu (`lgcode.json`)
3. Globalny katalog wtyczek (`~/.config/lgcode/plugins/`)
4. Katalog wtyczek projektu (`.lgcode/plugins/`)
...
lgcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z lgcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do lgcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej lgcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj lgcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do lgcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod lgcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
