...
opencode Desktop uruchamia w tle lokalny serwer opencode (sidecar `lgcode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/opencode/lgcode.jsonc` (lub `~/.config/opencode/lgcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/lgcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\lgcode.jsonc`
