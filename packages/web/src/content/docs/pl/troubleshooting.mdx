...
Aby debugować problemy z lgcode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\lgcode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `lgcode --log-level DEBUG`.

---

## Przechowywanie danych

lgcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\lgcode`
...
lgcode Desktop uruchamia w tle lokalny serwer lgcode (sidecar `lgcode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `LGcode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (lub `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\lgcode\lgcode.jsonc`
...
lgcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\lgcode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby lgcode mógł ją odbudować.

1. Całkowicie zamknij lgcode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/lgcode`
- **Linux**: usuń `~/.cache/lgcode` (lub uruchom `rm -rf ~/.cache/lgcode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\lgcode`

3. Uruchom ponownie lgcode Desktop.

---

### Napraw problemy z połączeniem z serwerem

lgcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `lgcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `LGCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `LGCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows lgcode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami lgcode.

---

### Brak powiadomień

lgcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla lgcode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie lgcode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji lgcode Desktop):

- `lgcode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `lgcode.global.dat` i `lgcode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `lgcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `lgcode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\lgcode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. lgcode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\lgcode`

2. Uruchom ponownie kod lgcode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to lgcode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
lgcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
