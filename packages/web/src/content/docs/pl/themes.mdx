...
Dzięki lgcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie lgcode używa naszego własnego motywu `lgcode`.
...
lgcode ma kilka wbudowanych motywów.
...
- Chcą, aby LGcode pasował do wyglądu ich terminala
...
lgcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/lgcode/themes/*.json` lub `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.lgcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.lgcode/themes/*.json`
