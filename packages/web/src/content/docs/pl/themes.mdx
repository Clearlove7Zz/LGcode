...
Dzięki loongcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie loongcode używa naszego własnego motywu `loongcode`.
...
loongcode ma kilka wbudowanych motywów.
...
- Chcą, aby Loongcode pasował do wyglądu ich terminala
...
loongcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/loongcode/themes/*.json` lub `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.loongcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.loongcode/themes/*.json`
