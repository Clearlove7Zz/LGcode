...
U Loongcode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Loongcode koristi nasu `loongcode` temu.
...
Loongcode dolazi sa vise ugradenih tema.
...
- Zele da Loongcode odgovara izgledu njihovog terminala
...
Loongcode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/loongcode/themes/*.json` ili `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.loongcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.loongcode/themes/*.json`
