...
U LGcode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, LGcode koristi nasu `lgcode` temu.
...
LGcode dolazi sa vise ugradenih tema.
...
- Zele da LGcode odgovara izgledu njihovog terminala
...
LGcode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/lgcode/themes/*.json` ili `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.lgcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.lgcode/themes/*.json`
