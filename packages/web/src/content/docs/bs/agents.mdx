...
Postoje dvije vrste agenata u LGcode: primarni agenti i podagenti.
...
LGcode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
LGcode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

LGcode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u LGcode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `lgcode.json`:
...
- Globalno: `~/.config/lgcode/agents/`
- Po projektu: `.lgcode/agents/`
...
Ako temperatura nije navedena, LGcode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu LGcode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj LGcode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [LGcode Zen](/docs/zen), koristili biste `lgcode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `lgcode models` da vidite listu dostupnih modela.
:::
