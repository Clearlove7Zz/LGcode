...
Agent skills omogucavaju Loongcode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Loongcode pretrazuje ove lokacije:

- Konfiguracija projekta: `.loongcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/loongcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Loongcode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.loongcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.loongcode/skills/git-release/SKILL.md` ovako:
...
Loongcode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `loongcode.json`:
...
**Za ugradene agente** (u `loongcode.json`):
