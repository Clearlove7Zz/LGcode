...
Agent skills omogucavaju LGcode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
LGcode pretrazuje ove lokacije:

- Konfiguracija projekta: `.lgcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/lgcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, LGcode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.lgcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.lgcode/skills/git-release/SKILL.md` ovako:
...
LGcode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `lgcode.json`:
...
**Za ugradene agente** (u `lgcode.json`):
