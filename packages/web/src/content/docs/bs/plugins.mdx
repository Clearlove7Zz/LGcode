---
title: Dodaci
description: Napišite vlastite dodatke za proširenje LGcode.
---

Dodaci vam omogućavaju da proširite LGcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja LGcode.
...
- `.lgcode/plugins/` - Dodaci na nivou projekta
- `~/.config/lgcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/lgcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/lgcode/lgcode.json`)
2. Konfiguracija projekta (`lgcode.json`)
3. Globalni direktorij dodataka (`~/.config/lgcode/plugins/`)
4. Direktorij dodataka projekta (`.lgcode/plugins/`)
...
LGcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje LGcode.
...
Spriječite lgcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u lgcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji lgcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za lgcode zajedno sa ugrađenim alatima.
