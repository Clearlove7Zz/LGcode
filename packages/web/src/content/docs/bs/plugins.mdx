---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Loongcode.
---

Dodaci vam omogućavaju da proširite Loongcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Loongcode.
...
- `.loongcode/plugins/` - Dodaci na nivou projekta
- `~/.config/loongcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/loongcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/loongcode/loongcode.json`)
2. Konfiguracija projekta (`loongcode.json`)
3. Globalni direktorij dodataka (`~/.config/loongcode/plugins/`)
4. Direktorij dodataka projekta (`.loongcode/plugins/`)
...
Loongcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Loongcode.
...
Spriječite loongcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u loongcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji loongcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za loongcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://modelhub.lgdg.cc/docs/sdk) za detalje.
