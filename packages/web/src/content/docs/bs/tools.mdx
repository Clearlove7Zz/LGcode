...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. LGcode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u LGcode.
...
:::note
Ovaj alat je dostupan samo kada je `LGCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `LGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz LGcode provajdera ili kada je varijabla `LGCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju LGcode:
