...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Loongcode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Loongcode.
...
:::note
Ovaj alat je dostupan samo kada je `LOONGCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `LOONGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Loongcode provajdera ili kada je varijabla `LOONGCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Loongcode:
