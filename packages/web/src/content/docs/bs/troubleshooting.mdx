...
Da biste otklonili probleme s LGcode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\lgcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `lgcode --log-level DEBUG`.

---

## Pohrana

LGcode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\lgcode`
...
LGcode Desktop pokreće lokalni LGcode server (`lgcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `LGcode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (ili `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\lgcode\lgcode.jsonc`
...
LGcode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\lgcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga LGcode mogao ponovo izgraditi.

1. Potpuno zatvorite LGcode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/lgcode`
- **Linux**: obrišite `~/.cache/lgcode` (ili pokrenite `rm -rf ~/.cache/lgcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\lgcode`

3. Ponovo pokrenite LGcode Desktop.

---

### Rješavanje problema sa vezom na serveru

LGcode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `lgcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LGCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LGCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, LGcode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa LGcode karakteristikama.

---

### Obavještenja se ne prikazuju

LGcode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za LGcode u postavkama vašeg OS-a, i
...
1. Zatvorite LGcode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka LGcode Desktop aplikacije):

- `lgcode.settings.dat` (URL zadanog servera za desktop)
- `lgcode.global.dat` i `lgcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `lgcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\lgcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. LGcode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\lgcode`

2. Ponovo pokrenite LGcode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti LGcode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
LGcode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
