...
Da biste otklonili probleme s Loongcode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\loongcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `loongcode --log-level DEBUG`.

---

## Pohrana

Loongcode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/loongcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\loongcode`
...
Loongcode Desktop pokreće lokalni Loongcode server (`loongcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Loongcode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/loongcode/loongcode.jsonc` (ili `~/.config/loongcode/loongcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/loongcode/loongcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\loongcode\loongcode.jsonc`
...
Loongcode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/loongcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\loongcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Loongcode mogao ponovo izgraditi.

1. Potpuno zatvorite Loongcode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/loongcode`
- **Linux**: obrišite `~/.cache/loongcode` (ili pokrenite `rm -rf ~/.cache/loongcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\loongcode`

3. Ponovo pokrenite Loongcode Desktop.

---

### Rješavanje problema sa vezom na serveru

Loongcode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `loongcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LOONGCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LOONGCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Loongcode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Loongcode karakteristikama.

---

### Obavještenja se ne prikazuju

Loongcode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Loongcode u postavkama vašeg OS-a, i
...
1. Zatvorite Loongcode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Loongcode Desktop aplikacije):

- `loongcode.settings.dat` (URL zadanog servera za desktop)
- `loongcode.global.dat` i `loongcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Loongcode:
...
Da saznate kojim modelima imate pristup, pokrenite `loongcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\loongcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Loongcode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\loongcode`

2. Ponovo pokrenite Loongcode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Loongcode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Loongcode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
