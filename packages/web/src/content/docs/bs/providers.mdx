---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u LGcode.
---

import config from "../../../../config.mjs"
export const console = config.console

LGcode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj LGcode konfiguraciji.
...
u `~/.local/share/lgcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem LGcode
config.
...
LGcode Zen je lista modela koje je obezbedio LGcode tim koji su bili
testirano i potvrđeno da dobro radi sa LGcode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa LGcode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite lgcode i idite na [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Radi kao i svaki drugi provajder u LGcode i potpuno je opcionalan za korištenje.

---

## LGcode Go

LGcode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža LGcode tim i koji su testirani i verificirani da dobro rade s LGcode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `LGcode Go` i idite na [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Da biste koristili Amazon Bedrock s LGcode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete lgcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `lgcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u LGcode.
:::
...
Možete konfigurirati lgcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi lgcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju lgcode konfiguraciju.
...
da pohrani token u lgcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
LGcode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate LGcode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`lgcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `lgcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s lgcode:
...
Za korištenje Google Vertex AI s LGcode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće lgcode.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) da automatski prijavite svaki LGcode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki LGcode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati lgcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati lgcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati lgcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za LGcode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/lgcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s LGcode:
...
3. Kopirajte API ključ za korištenje u LGcode.
...
6. **Važno**: Prije upotrebe modela oblaka u LGcode, morate lokalno povući informacije o modelu:
...
LGcode Zen je lista testiranih i verifikovanih modela koju je obezbedio LGcode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>LGcode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **LGcode Zen**.

```txt
   /connect
```

3. Unesite svoj LGcode API ključ.
...
Također možete dodati dodatne modele putem vaše lgcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše lgcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju lgcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s lgcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `lgcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju LGcode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `lgcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite LGcode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj lgcode konfiguraciji.
