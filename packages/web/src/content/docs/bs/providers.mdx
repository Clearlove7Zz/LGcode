---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Loongcode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Loongcode konfiguraciji.
...
u `~/.local/share/loongcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Loongcode
config.
...
Loongcode Zen je lista modela koje je obezbedio Loongcode tim koji su bili
testirano i potvrđeno da dobro radi sa Loongcode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Loongcode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite loongcode i idite na [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Radi kao i svaki drugi provajder u Loongcode i potpuno je opcionalan za korištenje.

---

## Loongcode Go

Loongcode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Loongcode tim i koji su testirani i verificirani da dobro rade s Loongcode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Loongcode Go` i idite na [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Da biste koristili Amazon Bedrock s Loongcode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete loongcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `loongcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Loongcode.
:::
...
Možete konfigurirati loongcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi loongcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju loongcode konfiguraciju.
...
da pohrani token u loongcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Loongcode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Loongcode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`loongcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `loongcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s loongcode:
...
Za korištenje Google Vertex AI s Loongcode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće loongcode.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) da automatski prijavite svaki Loongcode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Loongcode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati loongcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati loongcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati loongcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Loongcode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/loongcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Loongcode:
...
3. Kopirajte API ključ za korištenje u Loongcode.
...
6. **Važno**: Prije upotrebe modela oblaka u Loongcode, morate lokalno povući informacije o modelu:
...
Loongcode Zen je lista testiranih i verifikovanih modela koju je obezbedio Loongcode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Loongcode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Loongcode Zen**.

```txt
   /connect
```

3. Unesite svoj Loongcode API ključ.
...
Također možete dodati dodatne modele putem vaše loongcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše loongcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju loongcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s loongcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `loongcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Loongcode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `loongcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Loongcode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj loongcode konfiguraciji.
