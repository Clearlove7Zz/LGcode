...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/opencode/lgcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.lgcode` direktorij, i trebali bi pratiti istu strukturu.
