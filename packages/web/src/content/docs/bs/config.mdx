---
title: Konfiguracija
description: Korištenje Loongcode JSON konfiguracije.
---

Možete konfigurirati Loongcode koristeći JSON konfiguracijski fajl.

---

## Format

Loongcode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/loongcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/loongcode/loongcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LOONGCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`loongcode.json` u projektu) - postavke specifične za projekat
5. **`.loongcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LOONGCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.loongcode` i `~/.config/loongcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/loongcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Loongcode konfiguraciju u `~/.config/loongcode/loongcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/loongcode/tui.json`.
...
Dodajte `loongcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Loongcode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LOONGCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LOONGCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.loongcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.loongcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`modelhub.lgdg.cc/config.json`**](https://modelhub.lgdg.cc/config.json).

TUI konfiguracija koristi [**`modelhub.lgdg.cc/tui.json`**](https://modelhub.lgdg.cc/tui.json).
...
Koristite `LOONGCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `loongcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `loongcode serve` i `loongcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Loongcode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `loongcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Loongcode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Loongcode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/loongcode/agents/` ili `.loongcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Loongcode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`loongcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/loongcode/commands/` ili `.loongcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Loongcode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Loongcode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Loongcode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.loongcode/plugins/` ili `~/.config/loongcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Loongcode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
