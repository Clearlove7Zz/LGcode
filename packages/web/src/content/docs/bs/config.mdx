---
title: Konfiguracija
description: Korištenje LGcode JSON konfiguracije.
---

Možete konfigurirati LGcode koristeći JSON konfiguracijski fajl.

---

## Format

LGcode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/lgcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/lgcode/lgcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LGCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`lgcode.json` u projektu) - postavke specifične za projekat
5. **`.lgcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LGCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.lgcode` i `~/.config/lgcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/lgcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu LGcode konfiguraciju u `~/.config/lgcode/lgcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/lgcode/tui.json`.
...
Dodajte `lgcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se LGcode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LGCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LGCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.lgcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.lgcode`, tako da **može nadjačati** njihove postavke.
...
Koristite `LGCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `lgcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `lgcode serve` i `lgcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš LGcode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `lgcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj LGcode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, LGcode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/lgcode/agents/` ili `.lgcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, LGcode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`lgcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/lgcode/commands/` ili `.lgcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
LGcode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, LGcode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju LGcode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.lgcode/plugins/` ili `~/.config/lgcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite LGcode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
