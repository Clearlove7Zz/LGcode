...
Možete dodati vanjske alate u LGcode koristeći _Model Context Protocol_, ili MCP. LGcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [LGcode Config](https://modelhub.lgdg.cc/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/lgcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
LGcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, LGcode će:
...
Ako server zahtijeva autentifikaciju, LGcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `lgcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, LGcode sigurno čuva tokene u `~/.local/share/lgcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u LGcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz LGcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete LGcode sa vašim Sentry nalogom.
