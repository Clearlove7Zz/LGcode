...
Ako server zahtijeva autentifikaciju, LGcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, LGcode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
