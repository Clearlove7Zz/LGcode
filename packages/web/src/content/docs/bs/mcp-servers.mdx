...
Možete dodati vanjske alate u Loongcode koristeći _Model Context Protocol_, ili MCP. Loongcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Loongcode Config](https://modelhub.lgdg.cc/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/loongcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Loongcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Loongcode će:
...
Ako server zahtijeva autentifikaciju, Loongcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `loongcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Loongcode sigurno čuva tokene u `~/.local/share/loongcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Loongcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Loongcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Loongcode sa vašim Sentry nalogom.
