---
title: Server
description: Komunicirajte s loongcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `loongcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji loongcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `loongcode.local` |
...
Postavite `LOONGCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `loongcode`, ili postavite `LOONGCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `loongcode serve` i za `loongcode web`.
...
Kada pokrenete `loongcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite loongcode server za programsku interakciju sa loongcode-om.
:::

Ova arhitektura omogucava loongcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `loongcode serve` da startate standalone server. Ako je loongcode TUI vec pokrenut, `loongcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Loongcode [IDE](/docs/ide) pluginovi.
...
loongcode server izlaže sljedece API-je.
