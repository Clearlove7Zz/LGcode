---
title: Server
description: Komunicirajte s lgcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `lgcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji lgcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `lgcode.local` |
...
Postavite `LGCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `lgcode`, ili postavite `LGCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `lgcode serve` i za `lgcode web`.
...
Kada pokrenete `lgcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite lgcode server za programsku interakciju sa lgcode-om.
:::

Ova arhitektura omogucava lgcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `lgcode serve` da startate standalone server. Ako je lgcode TUI vec pokrenut, `lgcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste LGcode [IDE](/docs/ide) pluginovi.
...
lgcode server izlaže sljedece API-je.
