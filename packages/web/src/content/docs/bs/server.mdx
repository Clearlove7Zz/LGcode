...
Komanda `lgcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencode klijent moze koristiti.
...
Postavite `LGCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencode`, ili postavite `LGCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `lgcode serve` i za `lgcode web`.
...
Mozete pokrenuti `lgcode serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `lgcode serve` ce pokrenuti novi server.
