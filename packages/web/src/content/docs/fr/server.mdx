...
La commande `lgcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opencode peut utiliser.
...
Définissez `LGCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `opencode`, ou définissez `LGCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `lgcode serve` et à `lgcode web`.
...
opencode TUI en cours d'exécution, `lgcode serve` démarrera un nouveau serveur.
