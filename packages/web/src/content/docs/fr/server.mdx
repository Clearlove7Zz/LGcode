---
title: Serveur
description: Interagissez avec le serveur lgcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `lgcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client lgcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `lgcode.local` |
...
Définissez `LGCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `lgcode`, ou définissez `LGCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `lgcode serve` et à `lgcode web`.
...
Lorsque vous exécutez `lgcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur lgcode pour interagir avec lgcode par programmation.
:::

Cette architecture permet à lgcode de prendre en charge plusieurs clients et vous permet d'interagir avec lgcode par programme.

Vous pouvez exécuter `lgcode serve` pour démarrer un serveur autonome. Si vous avez le
lgcode TUI en cours d'exécution, `lgcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins LGcode [IDE](/docs/ide).
...
Le serveur lgcode expose les API suivantes.
