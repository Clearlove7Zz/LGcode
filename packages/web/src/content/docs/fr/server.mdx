---
title: Serveur
description: Interagissez avec le serveur loongcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `loongcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client loongcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `loongcode.local` |
...
Définissez `LOONGCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `loongcode`, ou définissez `LOONGCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `loongcode serve` et à `loongcode web`.
...
Lorsque vous exécutez `loongcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur loongcode pour interagir avec loongcode par programmation.
:::

Cette architecture permet à loongcode de prendre en charge plusieurs clients et vous permet d'interagir avec loongcode par programme.

Vous pouvez exécuter `loongcode serve` pour démarrer un serveur autonome. Si vous avez le
loongcode TUI en cours d'exécution, `loongcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Loongcode [IDE](/docs/ide).
...
Le serveur loongcode expose les API suivantes.
