...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Loongcode.
...
Voici plusieurs modèles qui fonctionnent bien avec Loongcode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Loongcode.

```json title="loongcode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Loongcode Zen](/docs/zen), vous utiliserez `loongcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="loongcode.jsonc" {7-12,19-24}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "provider": {
    "openai": {
...
```jsonc title="loongcode.jsonc" {6-21}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "provider": {
    "loongcode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Loongcode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Loongcode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="loongcode.jsonc" {7-18}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "provider": {
    "openai": {
...
Lorsque Loongcode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Loongcode.
