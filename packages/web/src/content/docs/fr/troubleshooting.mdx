...
Pour déboguer les problèmes avec Loongcode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\loongcode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `loongcode --log-level DEBUG`.

---

## Stockage

loongcode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/loongcode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\loongcode`
...
Loongcode Desktop exécute un serveur Loongcode local (le side-car `loongcode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Loongcode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/loongcode/loongcode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\loongcode\loongcode.jsonc`
...
Loongcode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/loongcode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\loongcode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Loongcode puisse le reconstruire.

1. Quittez complètement Loongcode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/loongcode`
- **Linux** : supprimez `~/.cache/loongcode` (ou exécutez `rm -rf ~/.cache/loongcode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\loongcode`

3. Redémarrez le bureau Loongcode.

---

### Résoudre les problèmes de connexion au serveur

Loongcode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `loongcode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `LOONGCODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `LOONGCODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Loongcode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Loongcode.

---

### Les notifications ne s'affichent pas

Loongcode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Loongcode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Loongcode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Loongcode Desktop) :

- `loongcode.settings.dat` (serveur de bureau par défaut URL)
- `loongcode.global.dat` et `loongcode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Loongcode :
...
3. Assurez-vous d'avoir la dernière version avec `loongcode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `loongcode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\loongcode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. loongcode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\loongcode`

2. Redémarrez loongcode pour réinstaller les derniers packages du fournisseur

Cela forcera loongcode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
loongcode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
