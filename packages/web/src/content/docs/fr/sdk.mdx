---
title: SDK
description: Client JS de type sécurisé pour le serveur loongcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK loongcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler loongcode par programme.
...
Créez une instance de loongcode :

```javascript
import { createOpencode } from "@loongcode-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `loongcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${loongcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de loongcode, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@loongcode-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@loongcode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*loongcode" },
})
