---
title: SDK
description: Client JS de type sécurisé pour le serveur lgcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK lgcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler lgcode par programme.
...
Créez une instance de lgcode :

```javascript
import { createOpencode } from "@lgcode-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `lgcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${lgcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de lgcode, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@lgcode-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@lgcode-ai/sdk"
```
