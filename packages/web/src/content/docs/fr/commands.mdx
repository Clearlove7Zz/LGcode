...
Vous pouvez ajouter des commandes personnalisées via la configuration LGcode ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre LGcode [config](/docs/config) :
...
lgcode comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
