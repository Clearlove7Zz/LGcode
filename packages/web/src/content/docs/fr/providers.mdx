---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Loongcode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Loongcode.
...
en `~/.local/share/loongcode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Loongcode
configuration.
...
Loongcode Zen est une liste de modèles fournis par l'équipe Loongcode qui ont été
testé et vérifié pour fonctionner correctement avec Loongcode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Loongcode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez loongcode et dirigez-vous vers [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Loongcode et son utilisation est totalement facultative.

---

## Loongcode Go

Loongcode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Loongcode qui ont été
testé et vérifié pour fonctionner correctement avec Loongcode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Loongcode Go` et rendez-vous sur [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Pour utiliser Amazon Bedrock avec Loongcode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de loongcode :
...
Pour une configuration spécifique au projet ou persistante, utilisez `loongcode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Loongcode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer loongcode pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que loongcode fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration loongcode.
...
pour stocker le jeton dans le stockage d'authentification loongcode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Loongcode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Loongcode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `loongcode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `loongcode.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec loongcode :
...
Pour utiliser Google Vertex AI avec Loongcode :
...
Définissez-les lors de l'exécution de loongcode.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via loongcode, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) pour enregistrer automatiquement chaque conversation Loongcode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Loongcode répertoriée comme une session distincte.
...
Vous pouvez configurer loongcode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer loongcode pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer loongcode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Loongcode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/loongcode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Loongcode :
...
3. Copiez la clé API à utiliser dans Loongcode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Loongcode, vous devez extraire les informations du modèle localement :
...
Loongcode Zen est une liste de modèles testés et vérifiés fournie par l'équipe Loongcode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Loongcode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Loongcode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Loongcode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration loongcode.
...
5. Vous pouvez également les personnaliser via votre configuration loongcode. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration loongcode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec loongcode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `loongcode.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Loongcode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `loongcode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration loongcode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration loongcode.
