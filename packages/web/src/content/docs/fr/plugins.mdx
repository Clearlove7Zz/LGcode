---
title: Plugins
description: Écrivez vos propres plugins pour étendre LGcode.
---

Les plugins vous permettent d'étendre LGcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de LGcode.
...
- `.lgcode/plugins/` - Plugins au niveau du projet
- `~/.config/lgcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/lgcode/node_modules/`.
...
1. Configuration globale (`~/.config/lgcode/lgcode.json`)
2. Configuration du projet (`lgcode.json`)
3. Répertoire global des plugins (`~/.config/lgcode/plugins/`)
4. Répertoire des plugins du projet (`.lgcode/plugins/`)
...
LGcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK lgcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre lgcode.
...
:::note
Si vous utilisez l'application de bureau LGcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez lgcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à lgcode :
...
L'assistant `tool` crée un outil personnalisé que lgcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour lgcode aux côtés des outils intégrés.
