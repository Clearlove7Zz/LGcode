---
title: Plugins
description: Écrivez vos propres plugins pour étendre Loongcode.
---

Les plugins vous permettent d'étendre Loongcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Loongcode.
...
- `.loongcode/plugins/` - Plugins au niveau du projet
- `~/.config/loongcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/loongcode/node_modules/`.
...
1. Configuration globale (`~/.config/loongcode/loongcode.json`)
2. Configuration du projet (`loongcode.json`)
3. Répertoire global des plugins (`~/.config/loongcode/plugins/`)
4. Répertoire des plugins du projet (`.loongcode/plugins/`)
...
Loongcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK loongcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre loongcode.
...
:::note
Si vous utilisez l'application de bureau Loongcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez loongcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à loongcode :
...
L'assistant `tool` crée un outil personnalisé que loongcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour loongcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://modelhub.lgdg.cc/docs/sdk) pour plus de détails.
