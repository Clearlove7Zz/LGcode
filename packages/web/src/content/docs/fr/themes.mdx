...
Avec Loongcode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Loongcode utilise notre propre thème `loongcode`.
...
Loongcode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Loongcode corresponde à l'apparence de leur terminal
...
Loongcode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/loongcode/themes/*.json` ou `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.loongcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.loongcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/loongcode/themes
vim ~/.config/loongcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .loongcode/themes
vim .loongcode/themes/my-theme.json
```
...
{
  "$schema": "https://modelhub.lgdg.cc/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
