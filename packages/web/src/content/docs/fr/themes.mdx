...
Avec LGcode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, LGcode utilise notre propre thème `lgcode`.
...
LGcode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que LGcode corresponde à l'apparence de leur terminal
...
LGcode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/lgcode/themes/*.json` ou `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.lgcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.lgcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/lgcode/themes
vim ~/.config/lgcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .lgcode/themes
vim .lgcode/themes/my-theme.json
```
