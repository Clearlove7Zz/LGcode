...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `lgcode.json` ou dans le `~/.config/opencode/lgcode.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
