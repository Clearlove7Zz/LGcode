...
Les compétences d'agent permettent à Loongcode de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Loongcode recherche ces emplacements :

- Configuration du projet : `.loongcode/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/loongcode/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Loongcode remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.loongcode/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.loongcode/skills/git-release/SKILL.md` comme ceci :
...
Loongcode répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `loongcode.json` :
...
**Pour les agents intégrés** (dans `loongcode.json`) :
