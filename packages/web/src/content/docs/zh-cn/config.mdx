...
2. **全局配置**（`~/.config/opencode/lgcode.json`）- 用户偏好
...
:::note
`.lgcode` 和 `~/.config/opencode` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
将全局 LGcode 配置放在 `~/.config/opencode/lgcode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.lgcode/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
您还可以使用 `~/.config/opencode/commands/` 或 `.lgcode/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.lgcode/plugins/` 或 `~/.config/opencode/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
