...
将全局 OpenCode 配置放在 `~/.config/opencode/lgcode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
使用 `OPENCODE_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.lgcode` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
