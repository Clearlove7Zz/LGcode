...
如果服务器需要身份验证，LGcode 会在你首次使用时提示你进行认证。你也可以使用 `opencode mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，LGcode 会将 Token 安全地存储在 `~/.local/share/opencode/mcp-auth.json` 中。
