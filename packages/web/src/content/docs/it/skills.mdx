...
Le skill degli agenti permettono a LGcode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
LGcode cerca in queste posizioni:

- Config di progetto: `.lgcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/lgcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, LGcode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.lgcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.lgcode/skills/git-release/SKILL.md` cosi':
...
LGcode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `lgcode.json`:
...
**Per agenti integrati** (in `lgcode.json`):
