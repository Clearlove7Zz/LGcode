...
Le skill degli agenti permettono a Loongcode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Loongcode cerca in queste posizioni:

- Config di progetto: `.loongcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/loongcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Loongcode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.loongcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.loongcode/skills/git-release/SKILL.md` cosi':
...
Loongcode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `loongcode.json`:
...
**Per agenti integrati** (in `loongcode.json`):
