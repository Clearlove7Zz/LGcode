...
2. **Config globale** (`~/.config/opencode/lgcode.json`) - preferenze utente
...
:::note
Le directory `.lgcode` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di LGcode in `~/.config/opencode/lgcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.lgcode/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.lgcode/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.lgcode/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
