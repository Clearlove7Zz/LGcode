...
Il comando `lgcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencode.
...
Imposta `LGCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `opencode`, oppure imposta `LGCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `lgcode serve` sia per `lgcode web`.
...
Puoi eseguire `lgcode serve` per avviare un server standalone. Se la TUI di opencode e gia in esecuzione, `lgcode serve` avviera un nuovo server.
