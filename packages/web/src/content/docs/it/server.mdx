---
title: Server
description: Interagisci con il server loongcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `loongcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client loongcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `loongcode.local` |
...
Imposta `LOONGCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `loongcode`, oppure imposta `LOONGCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `loongcode serve` sia per `loongcode web`.
...
Quando esegui `loongcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server loongcode per interagire con loongcode in modo programmatico.
:::

Questa architettura permette a loongcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `loongcode serve` per avviare un server standalone. Se la TUI di loongcode e gia in esecuzione, `loongcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Loongcode.
...
Il server loongcode espone le seguenti API.
