---
title: Server
description: Interagisci con il server lgcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `lgcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client lgcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `lgcode.local` |
...
Imposta `LGCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `lgcode`, oppure imposta `LGCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `lgcode serve` sia per `lgcode web`.
...
Quando esegui `lgcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server lgcode per interagire con lgcode in modo programmatico.
:::

Questa architettura permette a lgcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `lgcode serve` per avviare un server standalone. Se la TUI di lgcode e gia in esecuzione, `lgcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di LGcode.
...
Il server lgcode espone le seguenti API.
