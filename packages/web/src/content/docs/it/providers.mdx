---
title: Provider
description: Usa qualsiasi provider LLM in LGcode.
---

import config from "../../../../config.mjs"
export const console = config.console

LGcode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di LGcode.
...
LGcode Zen e una lista di modelli forniti dal team LGcode, testati e
verificati per funzionare bene con LGcode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da LGcode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona lgcode e vai su [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Funziona come qualsiasi altro provider in LGcode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con LGcode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui lgcode:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `lgcode.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in LGcode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare lgcode per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché lgcode funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di lgcode.
...
salvare il token nello storage di auth di lgcode.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
LGcode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare LGcode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `lgcode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth).

##### Configurazione

Personalizza tramite `lgcode.json`:
...
Per usare il tuo abbonamento GitHub Copilot con lgcode:
...
Per usare Google Vertex AI con LGcode:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui lgcode.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da lgcode, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) per loggare automaticamente ogni conversazione di LGcode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di LGcode elencata come sessione separata.
...
Puoi configurare lgcode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare lgcode per usare modelli locali tramite LM Studio.
...
Puoi configurare lgcode per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per LGcode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/lgcode) per dettagli.
:::
...
Per usare Ollama Cloud con LGcode:
...
3. Copia la chiave API da usare in LGcode.
...
6. **Importante**: Prima di usare modelli cloud in LGcode, devi scaricare le informazioni del modello localmente:
...
LGcode Zen è una lista di modelli testati e verificati forniti dal team LGcode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>LGcode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **LGcode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di LGcode.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di lgcode.
...
5. Puoi anche personalizzarli tramite la configurazione di lgcode. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di lgcode. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con lgcode. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `lgcode.json` nella directory del progetto:
...
I campi `limit` permettono a LGcode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `lgcode auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di lgcode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config lgcode.
