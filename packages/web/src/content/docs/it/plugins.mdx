---
title: Plugin
description: Scrivi plugin per estendere LGcode.
---

I plugin ti permettono di estendere LGcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di LGcode.
...
- `.lgcode/plugins/` - plugin a livello progetto
- `~/.config/lgcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/lgcode/node_modules/`.
...
1. Config globale (`~/.config/lgcode/lgcode.json`)
2. Config di progetto (`lgcode.json`)
3. Directory plugin globale (`~/.config/lgcode/plugins/`)
4. Directory plugin di progetto (`.lgcode/plugins/`)
...
LGcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di lgcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere lgcode.
...
:::note
Se usi l'app desktop di LGcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a lgcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a lgcode:
...
L'helper `tool` crea uno strumento personalizzato che lgcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in lgcode insieme agli strumenti integrati.
