---
title: Plugin
description: Scrivi plugin per estendere Loongcode.
---

I plugin ti permettono di estendere Loongcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Loongcode.
...
- `.loongcode/plugins/` - plugin a livello progetto
- `~/.config/loongcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/loongcode/node_modules/`.
...
1. Config globale (`~/.config/loongcode/loongcode.json`)
2. Config di progetto (`loongcode.json`)
3. Directory plugin globale (`~/.config/loongcode/plugins/`)
4. Directory plugin di progetto (`.loongcode/plugins/`)
...
Loongcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di loongcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere loongcode.
...
:::note
Se usi l'app desktop di Loongcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a loongcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a loongcode:
...
L'helper `tool` crea uno strumento personalizzato che loongcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in loongcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://modelhub.lgdg.cc/docs/sdk) per i dettagli.
