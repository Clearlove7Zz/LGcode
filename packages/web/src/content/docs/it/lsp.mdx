---
title: Server LSP
description: LGcode si integra con i tuoi server LSP.
---

LGcode può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

LGcode include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `LGCODE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e lgcode apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di lgcode.
