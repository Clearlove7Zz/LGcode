...
HTTP 基本認証でサーバーを保護するには、`LGCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencode` になるか、`LGCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`lgcode serve` と `lgcode web` の両方に当てはまります。
