...
サーバーが認証を必要とする場合、LGcode を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`opencode mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、LGcode はトークンを `~/.local/share/opencode/mcp-auth.json` に安全に保存します。
