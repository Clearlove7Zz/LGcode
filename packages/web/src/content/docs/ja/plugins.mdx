---
title: プラグイン
description: Loongcode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Loongcode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Loongcode のデフォルトの動作を変更したりできます。
...
- `.loongcode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/loongcode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/loongcode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/loongcode/loongcode.json`)
2. プロジェクト設定 (`loongcode.json`)
3. グローバルプラグインディレクトリ (`~/.config/loongcode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.loongcode/plugins/`)
...
Loongcode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Loongcode SDK クライアント。
...
ここでは、Loongcode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://modelhub.lgdg.cc/docs/sdk) を参照してください。
...
Loongcode が `.env` ファイルを読み取らないようにします。
...
プラグインは Loongcode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Loongcode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Loongcode で使用できます。
