...
Es gibt zwei Arten von Agenten in LGcode; Primäragenten und Subagenten.
...
LGcode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `lgcode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet LGcode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale LGcode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer LGcode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [LGcode Zen](/docs/zen) verwenden, würden Sie `lgcode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `lgcode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
