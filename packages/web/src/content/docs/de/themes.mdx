...
Mit Loongcode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Loongcode das `loongcode`-Theme.
...
Loongcode liefert mehrere eingebaute Themes mit.
...
- Loongcode optisch ans Terminal anpassen wollen
...
Loongcode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/loongcode/themes/*.json` oder `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.loongcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.loongcode/themes/*.json`
