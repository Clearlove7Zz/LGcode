...
Mit LGcode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet LGcode das `lgcode`-Theme.
...
LGcode liefert mehrere eingebaute Themes mit.
...
- LGcode optisch ans Terminal anpassen wollen
...
LGcode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/lgcode/themes/*.json` oder `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.lgcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.lgcode/themes/*.json`
