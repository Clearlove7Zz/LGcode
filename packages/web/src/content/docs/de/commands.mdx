...
Erstellen Sie `.loongcode/commands/test.md`:

```md title=".loongcode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Loongcode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Loongcode [config](/docs/config):

```json title="loongcode.jsonc" {4-12}
{
...
- Global: `~/.config/loongcode/commands/`
- Pro Projekt: `.loongcode/commands/`

```markdown title="~/.config/loongcode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".loongcode/commands/component.md"
---
...
```md title=".loongcode/commands/create-file.md"
---
...
```md title=".loongcode/commands/analyze-coverage.md"
---
...
```md title=".loongcode/commands/review-changes.md"
---
...
```md title=".loongcode/commands/review-component.md"
---
...
Loongcode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
