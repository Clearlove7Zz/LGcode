---
title: Server
description: Nutze den lgcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `lgcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein lgcode-Client nutzen kann.
...
Setze `LGCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `lgcode`, kann aber mit `LGCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `lgcode serve` und `lgcode web`.
...
Wenn du `lgcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den lgcode-Server, um lgcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `lgcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `lgcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die LGcode-[IDE](/docs/ide)-Plugins.
...
Der lgcode-Server stellt folgende APIs bereit.
