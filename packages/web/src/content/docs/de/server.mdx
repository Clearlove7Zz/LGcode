---
title: Server
description: Nutze den loongcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `loongcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein loongcode-Client nutzen kann.
...
Setze `LOONGCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `loongcode`, kann aber mit `LOONGCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `loongcode serve` und `loongcode web`.
...
Wenn du `loongcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den loongcode-Server, um loongcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `loongcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `loongcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Loongcode-[IDE](/docs/ide)-Plugins.
...
Der loongcode-Server stellt folgende APIs bereit.
