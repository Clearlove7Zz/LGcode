---
title: Regeln
description: Lege eigene Anweisungen fuer lgcode fest.
---

Du kannst lgcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in lgcode den Befehl `/init` aus.
...
So kann lgcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht lgcode sie zu erweitern.
...
lgcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/lgcode/AGENTS.md` speichern.
Diese gelten in allen lgcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt LGcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/lgcode/AGENTS.md` existiert)
...
Beim Start sucht lgcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/lgcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `lgcode.json` oder in `~/.config/lgcode/lgcode.json` angeben.
...
lgcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit lgcode.json

Empfohlen ist das `instructions`-Feld in der `lgcode.json`:
...
Du kannst lgcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `lgcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
