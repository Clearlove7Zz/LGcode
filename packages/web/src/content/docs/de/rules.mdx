---
title: Regeln
description: Lege eigene Anweisungen fuer loongcode fest.
---

Du kannst loongcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in loongcode den Befehl `/init` aus.
...
So kann loongcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht loongcode sie zu erweitern.
...
loongcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/loongcode/AGENTS.md` speichern.
Diese gelten in allen loongcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Loongcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/loongcode/AGENTS.md` existiert)
...
Beim Start sucht loongcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/loongcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `loongcode.json` oder in `~/.config/loongcode/loongcode.json` angeben.
...
loongcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit loongcode.json

Empfohlen ist das `instructions`-Feld in der `loongcode.json`:
...
Du kannst loongcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `loongcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
