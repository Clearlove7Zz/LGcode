...
Wenn LGcode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\lgcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `lgcode --log-level DEBUG`.

---

## Speicher

lgcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\lgcode`
...
LGcode Desktop startet im Hintergrund einen lokalen LGcode-Server (`lgcode-cli`-Sidecar).
...
- Nur macOS: `LGcode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (oder `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\lgcode\lgcode.jsonc`
...
LGcode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\lgcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit LGcode ihn neu aufbauen kann.

1. Beende LGcode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/lgcode`
- **Linux**: loesche `~/.cache/lgcode` (oder fuehre aus: `rm -rf ~/.cache/lgcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\lgcode`

3. Starte LGcode Desktop neu.

---

### Server-Verbindungsprobleme beheben

LGcode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `lgcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `LGCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `LGCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt LGcode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit LGcode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

LGcode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer LGcode in den OS-Einstellungen aktiviert sind, und
...
1. Beende LGcode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von LGcode Desktop):

- `lgcode.settings.dat` (desktop default server URL)
- `lgcode.global.dat` und `lgcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit LGcode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `lgcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\lgcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. lgcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\lgcode`

2. Starte lgcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt lgcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
lgcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
