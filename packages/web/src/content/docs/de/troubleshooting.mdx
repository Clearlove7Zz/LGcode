...
Wenn Loongcode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\loongcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `loongcode --log-level DEBUG`.

---

## Speicher

loongcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/loongcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\loongcode`
...
Loongcode Desktop startet im Hintergrund einen lokalen Loongcode-Server (`loongcode-cli`-Sidecar).
...
- Nur macOS: `Loongcode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/loongcode/loongcode.jsonc` (oder `~/.config/loongcode/loongcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/loongcode/loongcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\loongcode\loongcode.jsonc`
...
Loongcode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/loongcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\loongcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Loongcode ihn neu aufbauen kann.

1. Beende Loongcode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/loongcode`
- **Linux**: loesche `~/.cache/loongcode` (oder fuehre aus: `rm -rf ~/.cache/loongcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\loongcode`

3. Starte Loongcode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Loongcode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `loongcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `LOONGCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `LOONGCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Loongcode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Loongcode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Loongcode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Loongcode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Loongcode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Loongcode Desktop):

- `loongcode.settings.dat` (desktop default server URL)
- `loongcode.global.dat` und `loongcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Loongcode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `loongcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\loongcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. loongcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\loongcode`

2. Starte loongcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt loongcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
loongcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
