---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Loongcode zu erweitern.
---

Mit Plugins können Sie Loongcode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Loongcode zu ändern.
...
- `.loongcode/plugins/` – Plugins auf Projektebene
- `~/.config/loongcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/loongcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/loongcode/loongcode.json`)
2. Projektkonfiguration (`loongcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/loongcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.loongcode/plugins/`)
...
Loongcode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Loongcode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Loongcode erweitern können.
...
:::note
Wenn Sie die Loongcode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Loongcode `.env`-Dateien liest:
...
Plugins können Loongcode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Loongcode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Loongcode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://modelhub.lgdg.cc/docs/sdk).
