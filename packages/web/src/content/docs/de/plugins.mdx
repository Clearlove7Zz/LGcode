---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um LGcode zu erweitern.
---

Mit Plugins können Sie LGcode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von LGcode zu ändern.
...
- `.lgcode/plugins/` – Plugins auf Projektebene
- `~/.config/lgcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/lgcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/lgcode/lgcode.json`)
2. Projektkonfiguration (`lgcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/lgcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.lgcode/plugins/`)
...
LGcode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein LGcode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie LGcode erweitern können.
...
:::note
Wenn Sie die LGcode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass LGcode `.env`-Dateien liest:
...
Plugins können LGcode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das LGcode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für LGcode zur Verfügung.
