...
Agent Skills erlauben Loongcode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Loongcode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Loongcode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.loongcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.loongcode/skills/git-release/SKILL.md` zum Beispiel so:
...
Loongcode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `loongcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `loongcode.json`):
