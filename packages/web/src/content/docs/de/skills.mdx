...
Agent Skills erlauben LGcode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
LGcode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft LGcode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.lgcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.lgcode/skills/git-release/SKILL.md` zum Beispiel so:
...
LGcode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `lgcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `lgcode.json`):
