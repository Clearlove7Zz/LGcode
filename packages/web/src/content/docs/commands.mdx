...
Create `.loongcode/commands/test.md`:
...
You can add custom commands through the Loongcode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Loongcode [config](/docs/config):
...
- Per-project: `.loongcode/commands/`
...
loongcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
