...
Create `.lgcode/commands/test.md`:
...
You can add custom commands through the LGcode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your LGcode [config](/docs/config):
...
lgcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
