---
title: Config
description: Using the LGcode JSON config.
---

You can configure LGcode using a JSON config file.

---

## Format

LGcode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="lgcode.jsonc"
{
...
1. **Remote config** (from `.well-known/lgcode`) - organizational defaults
2. **Global config** (`~/.config/lgcode/lgcode.json`) - user preferences
3. **Custom config** (`LGCODE_CONFIG` env var) - custom overrides
4. **Project config** (`lgcode.json` in project) - project-specific settings
5. **`.lgcode` directories** - agents, commands, plugins
6. **Inline config** (`LGCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/lgcode/` on macOS) - admin-controlled
...
:::note
The `.lgcode` and `~/.config/lgcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/lgcode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/lgcode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global LGcode config in `~/.config/lgcode/lgcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/lgcode/tui.json`.
...
Add `lgcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When LGcode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `LGCODE_CONFIG` environment variable.

```bash
export LGCODE_CONFIG=/path/to/my/custom-config.json
lgcode run "Hello world"
```
...
Specify a custom config directory using the `LGCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.lgcode` directory, and should
follow the same structure.

```bash
export LGCODE_CONFIG_DIR=/path/to/my/config-directory
lgcode run "Hello world"
```

The custom directory is loaded after the global config and `.lgcode` directories, so it **can override** their settings.
...
Drop an `lgcode.json` or `lgcode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/lgcode/` |
| Linux    | `/etc/lgcode/`                         |
| Windows  | `%ProgramData%\lgcode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, LGcode reads managed preferences from the `ai.lgcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

LGcode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.lgcode.managed.plist`
2. `/Library/Managed Preferences/ai.lgcode.managed.plist`

The plist keys map directly to `lgcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.lgcode.managed` PayloadType. The LGcode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.lgcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.lgcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.lgcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `LGCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `lgcode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `lgcode serve` and `lgcode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your LGcode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `lgcode.local`. Useful for running multiple instances on the same network.
...
If not specified, LGcode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your LGcode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, LGcode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny LGcode actions on configured resources. Currently, policies can control which providers LGcode may use.
...
LGcode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, LGcode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/lgcode/agents/` or `.lgcode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, LGcode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`lgcode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/lgcode/commands/` or `.lgcode/commands/`. [Learn more here](/docs/commands).
...
LGcode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
LGcode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, lgcode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend LGcode with custom tools, hooks, and integrations.

Place plugin files in `.lgcode/plugins/` or `~/.config/lgcode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="lgcode.json"
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "plugin": ["lgcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict LGcode to only use specific providers rather than disabling them one by one.
