...
Place your global OpenCode config in `~/.config/opencode/lgcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
On macOS, OpenCode reads managed preferences from the `ai.lgcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.
...
Use the `ai.lgcode.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
