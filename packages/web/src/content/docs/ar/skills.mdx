...
تتيح مهارات الوكيل لـ Loongcode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Loongcode في هذه المواقع:

- إعدادات المشروع: `.loongcode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/loongcode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Loongcode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.loongcode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/loongcode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.loongcode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Loongcode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `loongcode.json`:
...
**للوكلاء المدمجين** (في `loongcode.json`):
