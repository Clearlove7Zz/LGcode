---
title: المزوّدون
description: استخدام أي موفّر LLM في LGcode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم LGcode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات LGcode.
...
في `~/.local/share/lgcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات LGcode.
...
LGcode Zen هي قائمة نماذج يوفّرها فريق LGcode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع LGcode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع LGcode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر lgcode، ثم انتقل إلى [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
يعمل مثل أي مزوّد آخر في LGcode واستخدامه اختياري بالكامل.

---

## LGcode Go

LGcode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق LGcode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع LGcode.

1. شغّل الأمر `/connect` في TUI، واختر `LGcode Go`، ثم انتقل إلى [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
لاستخدام Amazon Bedrock مع LGcode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل lgcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `lgcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل LGcode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين lgcode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل lgcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات lgcode.
...
تخزين الرمز في مساحة تخزين مصادقة lgcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم LGcode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر LGcode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`lgcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth).

##### التهيئة

خصّص عبر `lgcode.json`:
...
لاستخدام اشتراك GitHub Copilot مع lgcode:
...
لاستخدام Google Vertex AI مع LGcode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر lgcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) لتسجيل كل محادثة LGcode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة LGcode مدرجة كجلسة مستقلة.
...
يمكنك ضبط lgcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط lgcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط lgcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع LGcode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/lgcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع LGcode:
...
3. انسخ مفتاح API لاستخدامه في LGcode.
...
6. **مهم**: قبل استخدام نماذج السحابة في LGcode، يجب سحب معلومات النموذج محليا:
...
LGcode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق LGcode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>LGcode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **LGcode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ LGcode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات lgcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات lgcode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات lgcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع lgcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `lgcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ LGcode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `lgcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات lgcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات lgcode.
