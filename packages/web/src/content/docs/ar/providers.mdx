---
title: المزوّدون
description: استخدام أي موفّر LLM في Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Loongcode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Loongcode.
...
في `~/.local/share/loongcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Loongcode.
...
Loongcode Zen هي قائمة نماذج يوفّرها فريق Loongcode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Loongcode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Loongcode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر loongcode، ثم انتقل إلى [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
يعمل مثل أي مزوّد آخر في Loongcode واستخدامه اختياري بالكامل.

---

## Loongcode Go

Loongcode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Loongcode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Loongcode.

1. شغّل الأمر `/connect` في TUI، واختر `Loongcode Go`، ثم انتقل إلى [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
لاستخدام Amazon Bedrock مع Loongcode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل loongcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `loongcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Loongcode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين loongcode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل loongcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات loongcode.
...
تخزين الرمز في مساحة تخزين مصادقة loongcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Loongcode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Loongcode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`loongcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع loongcode:
...
لاستخدام Google Vertex AI مع Loongcode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر loongcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) لتسجيل كل محادثة Loongcode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Loongcode مدرجة كجلسة مستقلة.
...
يمكنك ضبط loongcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط loongcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط loongcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Loongcode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/loongcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Loongcode:
...
3. انسخ مفتاح API لاستخدامه في Loongcode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Loongcode، يجب سحب معلومات النموذج محليا:
...
Loongcode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Loongcode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Loongcode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Loongcode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Loongcode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات loongcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات loongcode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات loongcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع loongcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `loongcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Loongcode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `loongcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات loongcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات loongcode.
