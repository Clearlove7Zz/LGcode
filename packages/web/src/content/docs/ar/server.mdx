...
يشغّل الأمر `lgcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opencode استخدامها.
...
عيّن `LGCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `opencode`، أو عيّن `LGCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `lgcode serve` و `lgcode web`.
...
يمكنك تشغيل `lgcode serve` لبدء خادم مستقل. إذا كانت واجهة opencode في terminal (TUI)
