...
مع LGcode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم LGcode سمتنا `lgcode`.
...
- يريدون أن يطابق LGcode مظهر terminal لديهم
...
يدعم LGcode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/lgcode/themes/*.json` أو `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.lgcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.lgcode/themes/*.json`
