...
مع Loongcode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Loongcode سمتنا `loongcode`.
...
يأتي Loongcode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Loongcode مظهر terminal لديهم
...
يدعم Loongcode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/loongcode/themes/*.json` أو `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.loongcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.loongcode/themes/*.json`
