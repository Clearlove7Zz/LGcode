...
أنشئ الملف `.lgcode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات LGcode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ LGcode:
...
- على مستوى النظام: `~/.config/lgcode/commands/`
- لكل مشروع: `.lgcode/commands/`
...
يتضمن lgcode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
