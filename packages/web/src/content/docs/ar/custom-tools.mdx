...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في loongcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في loongcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.loongcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/loongcode/tools/`.
