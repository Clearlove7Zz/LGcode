...
Com o lgcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o lgcode usa nosso próprio tema `lgcode`.
...
O lgcode vem com vários temas embutidos.
...
- Querem que o lgcode corresponda à aparência do seu terminal
...
O lgcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/lgcode/themes/*.json` ou `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.lgcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.lgcode/themes/*.json`
