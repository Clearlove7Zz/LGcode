...
Com o loongcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o loongcode usa nosso próprio tema `loongcode`.
...
O loongcode vem com vários temas embutidos.
...
- Querem que o loongcode corresponda à aparência do seu terminal
...
O loongcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/loongcode/themes/*.json` ou `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.loongcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.loongcode/themes/*.json`
