...
Você pode adicionar ferramentas externas ao lgcode usando o _Model Context Protocol_, ou MCP. O lgcode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do lgcode](https://modelhub.lgdg.cc/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/lgcode`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O lgcode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o lgcode irá:
...
Se o servidor requer autenticação, o lgcode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `lgcode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o lgcode armazenará os tokens de forma segura em `~/.local/share/lgcode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no lgcode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do lgcode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o lgcode à sua conta do Sentry.
