---
title: Servidor
description: Interaja com o servidor loongcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `loongcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente loongcode pode usar.
...
```bash
loongcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `loongcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
loongcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LOONGCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `loongcode`, ou defina `LOONGCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `loongcode serve` quanto ao `loongcode web`.

```bash
LOONGCODE_SERVER_PASSWORD=your-password loongcode serve
```
...
Quando você executa `loongcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor loongcode para interagir com o loongcode programaticamente.
:::

Essa arquitetura permite que o loongcode suporte múltiplos clientes e permite que você interaja com o loongcode programaticamente.

Você pode executar `loongcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do loongcode em execução, `loongcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do loongcode [IDE](/docs/ide).
...
O servidor loongcode expõe as seguintes APIs.
