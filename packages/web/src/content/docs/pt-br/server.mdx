---
title: Servidor
description: Interaja com o servidor lgcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `lgcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente lgcode pode usar.
...
```bash
lgcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `lgcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
lgcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `LGCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `lgcode`, ou defina `LGCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `lgcode serve` quanto ao `lgcode web`.

```bash
LGCODE_SERVER_PASSWORD=your-password lgcode serve
```
...
Quando você executa `lgcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor lgcode para interagir com o lgcode programaticamente.
:::

Essa arquitetura permite que o lgcode suporte múltiplos clientes e permite que você interaja com o lgcode programaticamente.

Você pode executar `lgcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do lgcode em execução, `lgcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do lgcode [IDE](/docs/ide).
...
O servidor lgcode expõe as seguintes APIs.
