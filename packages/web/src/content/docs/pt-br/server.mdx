...
O comando `lgcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencode pode usar.
...
Defina `LGCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `opencode`, ou defina `LGCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `lgcode serve` quanto ao `lgcode web`.
...
Você pode executar `lgcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencode em execução, `lgcode serve` iniciará um novo servidor.
