...
Existem dois tipos de agentes no loongcode; agentes primários e subagentes.
...
loongcode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Loongcode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Loongcode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Loongcode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `loongcode.json`:
...
- Global: `~/.config/loongcode/agents/`
- Por projeto: `.loongcode/agents/`

```markdown title="~/.config/loongcode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o loongcode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do loongcode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do loongcode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Loongcode Zen](/docs/zen), você usaria `loongcode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="loongcode.json" {3-6,9-12}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "tools": {
    "write": true,
...
```json title="loongcode.json" {3-5,8-10}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `loongcode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/Clearlove7Zz/LoongCode).
:::
...
```markdown title="~/.config/loongcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/loongcode/agents/security-auditor.md"
---
