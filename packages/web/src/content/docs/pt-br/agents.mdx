...
Existem dois tipos de agentes no lgcode; agentes primários e subagentes.
...
lgcode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
LGcode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

LGcode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do LGcode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `lgcode.json`:
...
- Global: `~/.config/lgcode/agents/`
- Por projeto: `.lgcode/agents/`

```markdown title="~/.config/lgcode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o lgcode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do lgcode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do lgcode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [LGcode Zen](/docs/zen), você usaria `lgcode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="lgcode.json" {3-6,9-12}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "tools": {
    "write": true,
...
```json title="lgcode.json" {3-5,8-10}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `lgcode models` para ver uma lista dos modelos disponíveis.
:::
...
```markdown title="~/.config/lgcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/lgcode/agents/security-auditor.md"
---
