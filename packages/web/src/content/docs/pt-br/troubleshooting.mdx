...
O opencode Desktop executa um servidor opencode local (o sidecar `lgcode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/lgcode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\lgcode.jsonc`
