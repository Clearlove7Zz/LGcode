---
title: Plugins
description: Escreva seus próprios plugins para estender o loongcode.
---

Plugins permitem que você estenda o loongcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do loongcode.
...
- `.loongcode/plugins/` - Plugins em nível de projeto
- `~/.config/loongcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/loongcode/node_modules/`.
...
1. Configuração global (`~/.config/loongcode/loongcode.json`)
2. Configuração do projeto (`loongcode.json`)
3. Diretório de plugins global (`~/.config/loongcode/plugins/`)
4. Diretório de plugins do projeto (`.loongcode/plugins/`)
...
O loongcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do loongcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o loongcode.
...
:::note
Se você estiver usando o aplicativo desktop loongcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o loongcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao loongcode:
...
O helper `tool` cria uma ferramenta personalizada que o loongcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o loongcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://modelhub.lgdg.cc/docs/sdk) para detalhes.
