---
title: Plugins
description: Escreva seus próprios plugins para estender o lgcode.
---

Plugins permitem que você estenda o lgcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do lgcode.
...
- `.lgcode/plugins/` - Plugins em nível de projeto
- `~/.config/lgcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/lgcode/node_modules/`.
...
1. Configuração global (`~/.config/lgcode/lgcode.json`)
2. Configuração do projeto (`lgcode.json`)
3. Diretório de plugins global (`~/.config/lgcode/plugins/`)
4. Diretório de plugins do projeto (`.lgcode/plugins/`)
...
O lgcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do lgcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o lgcode.
...
:::note
Se você estiver usando o aplicativo desktop lgcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o lgcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao lgcode:
...
O helper `tool` cria uma ferramenta personalizada que o lgcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o lgcode junto com as ferramentas integradas.
