...
Você pode especificar arquivos de instrução personalizados no seu `lgcode.json` ou no global `~/.config/opencode/lgcode.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
