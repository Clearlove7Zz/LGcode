...
As habilidades do agente permitem que o lgcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O lgcode pesquisa nesses locais:

- Configuração do projeto: `.lgcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/lgcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o lgcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.lgcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.lgcode/skills/git-release/SKILL.md` assim:
...
O lgcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `lgcode.json`:
...
**Para agentes embutidos** (em `lgcode.json`):
