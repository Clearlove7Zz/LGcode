...
As habilidades do agente permitem que o loongcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O loongcode pesquisa nesses locais:

- Configuração do projeto: `.loongcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/loongcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o loongcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.loongcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.loongcode/skills/git-release/SKILL.md` assim:
...
O loongcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `loongcode.json`:
...
**Para agentes embutidos** (em `loongcode.json`):
