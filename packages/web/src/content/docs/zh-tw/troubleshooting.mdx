...
- **macOS/Linux**: `~/.config/opencode/lgcode.jsonc`（或 `~/.config/opencode/lgcode.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/lgcode.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\lgcode.jsonc`
