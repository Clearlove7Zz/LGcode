...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. loongcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
loongcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `LOONGCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `LOONGCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca loongcode provider kullanılırken veya `LOONGCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

loongcode başlatırken etkinleştirmek için:
