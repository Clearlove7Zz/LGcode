...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. lgcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
lgcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `LGCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `LGCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca lgcode provider kullanılırken veya `LGCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

lgcode başlatırken etkinleştirmek için:
