...
_Model Bağlam Protokolü_ veya MCP kullanarak lgcode'a harici araçlar ekleyebilirsiniz. lgcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [lgcode Config](https://modelhub.lgdg.cc/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/lgcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
lgcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde lgcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa lgcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `lgcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından lgcode, belirteçleri `~/.local/share/lgcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra lgcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi lgcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve lgcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
