...
_Model Bağlam Protokolü_ veya MCP kullanarak loongcode'a harici araçlar ekleyebilirsiniz. loongcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [loongcode Config](https://modelhub.lgdg.cc/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/loongcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
loongcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde loongcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa loongcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `loongcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından loongcode, belirteçleri `~/.local/share/loongcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra loongcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi loongcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve loongcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
