...
Loongcode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Loongcode'daki diğer sağlayıcılar gibi çalışır. Loongcode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Loongcode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Loongcode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Loongcode Go, Loongcode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Loongcode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Loongcode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Loongcode Go'ya abone olabilir.
:::
...
Loongcode Go aşağıdaki limitleri içerir:
...
Loongcode yapılandırmanızdaki [model id](/docs/config/#models) formatı `loongcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `loongcode-go/kimi-k2.7-code` kullanmalısınız.
...
Loongcode Go'yu şu amaçlarla oluşturduk:
...
4. Loongcode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
