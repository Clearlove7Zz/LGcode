...
LGcode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, LGcode'daki diğer sağlayıcılar gibi çalışır. LGcode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve LGcode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
LGcode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

LGcode Go, LGcode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>LGcode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `LGcode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye LGcode Go'ya abone olabilir.
:::
...
LGcode Go aşağıdaki limitleri içerir:
...
LGcode yapılandırmanızdaki [model id](/docs/config/#models) formatı `lgcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `lgcode-go/kimi-k2.7-code` kullanmalısınız.
...
LGcode Go'yu şu amaçlarla oluşturduk:
...
4. LGcode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
