...
Ajan becerileri, loongcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
loongcode su konumlari tarar:

- Proje konfigurasyonu: `.loongcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/loongcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda loongcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.loongcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.loongcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
loongcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`loongcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`loongcode.json` icinde):
