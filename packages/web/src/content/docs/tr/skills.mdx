...
Ajan becerileri, lgcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
lgcode su konumlari tarar:

- Proje konfigurasyonu: `.lgcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/lgcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda lgcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.lgcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.lgcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
lgcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`lgcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`lgcode.json` icinde):
