...
loongcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak loongcode kendi `loongcode` temasını kullanır.
...
loongcode birden fazla yerleşik temayla gelir.
...
- loongcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
loongcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/loongcode/themes/*.json` veya `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.loongcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.loongcode/themes/*.json`
