...
lgcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak lgcode kendi `lgcode` temasını kullanır.
...
lgcode birden fazla yerleşik temayla gelir.
...
- lgcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
lgcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/lgcode/themes/*.json` veya `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.lgcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.lgcode/themes/*.json`
