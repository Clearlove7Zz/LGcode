---
title: Eklentiler
description: Loongcode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Loongcode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Loongcode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.loongcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/loongcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/loongcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/loongcode/loongcode.json`)
2. Project config (`loongcode.json`)
3. Global eklenti dizini (`~/.config/loongcode/plugins/`)
4. Proje eklenti dizini (`.loongcode/plugins/`)
...
Loongcode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Loongcode SDK istemcisi.
...
Loongcode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Loongcode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Loongcode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Loongcode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Loongcode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Loongcode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://modelhub.lgdg.cc/docs/sdk) bakın.
