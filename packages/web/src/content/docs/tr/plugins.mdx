---
title: Eklentiler
description: LGcode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek LGcode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya LGcode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.lgcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/lgcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/lgcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/lgcode/lgcode.json`)
2. Project config (`lgcode.json`)
3. Global eklenti dizini (`~/.config/lgcode/plugins/`)
4. Proje eklenti dizini (`.lgcode/plugins/`)
...
LGcode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir LGcode SDK istemcisi.
...
LGcode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
LGcode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
LGcode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca LGcode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, LGcode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra LGcode'a da hazır olacaktır.
