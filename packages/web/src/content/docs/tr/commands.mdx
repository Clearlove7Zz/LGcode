...
`.loongcode/commands/test.md` oluştur:
...
loongcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

loongcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/loongcode/commands/`
- Proje başına: `.loongcode/commands/`
...
loongcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
