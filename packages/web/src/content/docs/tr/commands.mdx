...
`.lgcode/commands/test.md` oluştur:
...
lgcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

lgcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/lgcode/commands/`
- Proje başına: `.lgcode/commands/`
...
lgcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
