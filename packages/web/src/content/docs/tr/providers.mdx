...
LGcode Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
...
1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth)'ye gidin.
...
LGcode Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her opencode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) eklentisini kullanın.
...
LGcode Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
