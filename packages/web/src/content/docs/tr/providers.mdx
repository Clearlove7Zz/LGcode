---
title: Sağlayıcılar
description: lgcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

lgcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı lgcode yapılandırmanızda yapılandırın.
...
`~/.local/share/lgcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları lgcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
LGcode Zen, lgcode ekibi tarafından sağlanan modellerin bir listesidir.
lgcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz LGcode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, lgcode'u seçin ve [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth)'ye gidin.
...
lgcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## LGcode Go

LGcode Go, lgcode ile iyi çalıştığı test edilmiş ve doğrulanmış, lgcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `LGcode Go`'yu seçin ve [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen) adresine gidin.
...
Amazon Bedrock'u lgcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   lgcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `lgcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi lgcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
lgcode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   lgcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
lgcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. lgcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`lgcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`lgcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi lgcode'la kullanmak için:
...
Google Vertex AI'yi lgcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları lgcode çalıştırılırken ayarlayın.
...
Helicone'un lgcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her lgcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her lgcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
lgcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla lgcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla lgcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini lgcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/lgcode)'a bakın.
:::
...
Ollama Cloud'u lgcode ile kullanmak için:
...
3. lgcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: lgcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
LGcode Zen, lgcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>LGcode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **LGcode Zen**'i arayın.

   ```txt
   /connect
   ```

3. lgcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   lgcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca lgcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri lgcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı lgcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `lgcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları lgcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `lgcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için lgcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, lgcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
