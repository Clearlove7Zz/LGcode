---
title: Sağlayıcılar
description: loongcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

loongcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı loongcode yapılandırmanızda yapılandırın.
...
`~/.local/share/loongcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları loongcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Loongcode Zen, loongcode ekibi tarafından sağlanan modellerin bir listesidir.
loongcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Loongcode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, loongcode'u seçin ve [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth)'ye gidin.
...
loongcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Loongcode Go

Loongcode Go, loongcode ile iyi çalıştığı test edilmiş ve doğrulanmış, loongcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Loongcode Go`'yu seçin ve [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen) adresine gidin.
...
Amazon Bedrock'u loongcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   loongcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `loongcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi loongcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
loongcode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   loongcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
loongcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. loongcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`loongcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`loongcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi loongcode'la kullanmak için:
...
Google Vertex AI'yi loongcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları loongcode çalıştırılırken ayarlayın.
...
Helicone'un loongcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her loongcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her loongcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
loongcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla loongcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla loongcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini loongcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/loongcode)'a bakın.
:::
...
Ollama Cloud'u loongcode ile kullanmak için:
...
3. loongcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: loongcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Loongcode Zen, loongcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Loongcode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Loongcode Zen**'i arayın.

   ```txt
   /connect
   ```

3. loongcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   loongcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca loongcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri loongcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı loongcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `loongcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları loongcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `loongcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için loongcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, loongcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
