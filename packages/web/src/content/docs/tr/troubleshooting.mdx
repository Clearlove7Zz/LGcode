...
lgcode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\lgcode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `lgcode --log-level DEBUG`.

---

## Depolama

lgcode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\lgcode` yapistirin
...
lgcode Desktop arka planda yerel bir lgcode sunucusu (`lgcode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `lgcode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (veya `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\lgcode\lgcode.jsonc` yapistirin
...
lgcode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\lgcode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), lgcode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. lgcode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/lgcode`
- **Linux**: `~/.cache/lgcode` dizinini silin (veya `rm -rf ~/.cache/lgcode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\lgcode` yapistirin

3. lgcode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

lgcode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`lgcode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `LGCODE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `LGCODE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta lgcode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, lgcode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

lgcode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda lgcode icin bildirimler etkinse
...
1. lgcode Desktop'u kapatin
2. Su dosyalari bulun ve silin (lgcode Desktop uygulama veri dizininde yer alirlar):

- `lgcode.settings.dat` (desktop varsayilan sunucu URL'si)
- `lgcode.global.dat` ve `lgcode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
lgcode ile ilgili bir sorun yasiyorsaniz:
...
3. `lgcode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `lgcode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\lgcode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. lgcode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\lgcode`

2. En guncel provider paketlerini yeniden kurmak icin lgcode'u yeniden baslatin

Bu, lgcode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
lgcode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
