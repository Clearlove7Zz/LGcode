...
opencode Desktop arka planda yerel bir opencode sunucusu (`lgcode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.
...
- **macOS/Linux**: `~/.config/opencode/lgcode.jsonc` (veya `~/.config/opencode/lgcode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/lgcode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\lgcode.jsonc` yapistirin
