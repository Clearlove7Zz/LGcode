...
loongcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
loongcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Loongcode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Loongcode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `loongcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse loongcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global loongcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
loongcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Loongcode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `loongcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `loongcode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/Clearlove7Zz/LoongCode).
:::
