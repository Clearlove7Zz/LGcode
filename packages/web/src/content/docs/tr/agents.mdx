...
lgcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
lgcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
LGcode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

LGcode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `lgcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse lgcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global lgcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
lgcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [LGcode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `lgcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `lgcode models` komutunu çalıştırın.
:::
