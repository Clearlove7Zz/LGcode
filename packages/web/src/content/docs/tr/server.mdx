---
title: Sunucu
description: loongcode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`loongcode serve` komutu, loongcode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `loongcode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LOONGCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `loongcode` değeridir; değiştirmek isterseniz `LOONGCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `loongcode serve` hem de `loongcode web` için geçerlidir.
...
`loongcode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
loongcode ile programatik etkileşim için loongcode sunucusunu kullanın.
:::

Bu mimari, loongcode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`loongcode serve` ile bağımsız bir sunucu başlatabilirsiniz. loongcode TUI açıksa, `loongcode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum loongcode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
loongcode sunucusu aşağıdaki API'leri sunar.
