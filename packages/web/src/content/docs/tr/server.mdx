...
`lgcode serve` komutu, opencode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `LGCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `LGCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `lgcode serve` hem de `lgcode web` için geçerlidir.
...
`lgcode serve` ile bağımsız bir sunucu başlatabilirsiniz. opencode TUI açıksa, `lgcode serve` yeni bir sunucu başlatır.
