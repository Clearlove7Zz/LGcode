---
title: LSP Servers
description: Loongcode integrates with your LSP servers.
---

Loongcode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Loongcode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `LOONGCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and loongcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your loongcode config.
...
```json title="loongcode.json" {5-13}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "lsp": {
    "custom-lsp": {
