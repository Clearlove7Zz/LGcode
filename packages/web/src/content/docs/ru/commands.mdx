...
Создайте `.loongcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию loongcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем loongcode [config](/docs/config):
...
- Глобальный: `~/.config/loongcode/commands/`
- Для каждого проекта: `.loongcode/commands/`
...
loongcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
