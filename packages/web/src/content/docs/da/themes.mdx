...
Med Loongcode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Loongcode vårt eget `loongcode`-tema.
...
Loongcode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/loongcode/themes/*.json` eller `$XDG_CONFIG_HOME/loongcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.loongcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.loongcode/themes/*.json`
