...
Med LGcode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger LGcode vårt eget `lgcode`-tema.
...
LGcode kommer med flere innebygde temaer.
...
- Vil at LGcode skal matche terminalens utseende
...
LGcode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/lgcode/themes/*.json` eller `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.lgcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.lgcode/themes/*.json`
