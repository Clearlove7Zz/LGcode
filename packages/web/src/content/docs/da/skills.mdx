...
Agentferdigheter lar LGcode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
LGcode søker etter disse stedene:

- Prosjektkonfigurasjon: `.lgcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/lgcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går LGcode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.lgcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.lgcode/skills/git-release/SKILL.md` slik:
...
LGcode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `lgcode.json`:
...
**For innebygde agenter** (i `lgcode.json`):
