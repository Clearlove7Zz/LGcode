...
Hvis serveren kræver godkendelse, vil LGcode bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil LGcode opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
