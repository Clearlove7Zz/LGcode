...
Du kan tilføje eksterne værktøjer til Loongcode ved hjælp af _Model Context Protocol_ eller MCP. Loongcode understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Loongcode Config](https://modelhub.lgdg.cc/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/loongcode`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Loongcode håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Loongcode:
...
Hvis serveren kræver godkendelse, vil Loongcode bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `loongcode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Loongcode opbevare tokens sikkert i `~/.local/share/loongcode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Loongcode-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Loongcode til din Sentry-konto.
