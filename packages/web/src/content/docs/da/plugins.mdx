---
title: Plugins
description: Skriv dine egne plugins for at udvide Loongcode.
---

Plugins giver dig mulighed for at udvide Loongcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Loongcodes standardadfærd.
...
- `.loongcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/loongcode/node_modules/`.
...
1. Global konfiguration (`~/.config/loongcode/loongcode.json`)
2. Projektkonfiguration (`loongcode.json`)
3. Global plugin-mappe (`~/.config/loongcode/plugins/`)
4. Projekt plugin bibliotek (`.loongcode/plugins/`)
...
Loongcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En loongcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide loongcode.
...
:::note
Hvis du bruger Loongcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå loongcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til loongcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som loongcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for loongcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://modelhub.lgdg.cc/docs/sdk) for detaljer.
