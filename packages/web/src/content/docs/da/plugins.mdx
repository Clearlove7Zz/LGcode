---
title: Plugins
description: Skriv dine egne plugins for at udvide LGcode.
---

Plugins giver dig mulighed for at udvide LGcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre LGcodes standardadfærd.
...
- `.lgcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/lgcode/node_modules/`.
...
1. Global konfiguration (`~/.config/lgcode/lgcode.json`)
2. Projektkonfiguration (`lgcode.json`)
3. Global plugin-mappe (`~/.config/lgcode/plugins/`)
4. Projekt plugin bibliotek (`.lgcode/plugins/`)
...
LGcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En lgcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide lgcode.
...
:::note
Hvis du bruger LGcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå lgcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til lgcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som lgcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for lgcode sammen med indbyggede værktøjer.
