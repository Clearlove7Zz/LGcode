---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i LGcode.
---

import config from "../../../../config.mjs"
export const console = config.console

LGcode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i LGcode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/lgcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i LGcode-konfigurationen.
...
LGcode Zen er en liste over modeller leveret af LGcode-teamet, som er blevet testet og verificeret til at fungere godt med LGcode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med LGcode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg lgcode og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Det fungerer som alle andre udbydere i LGcode og er helt valgfrit at bruge.

---

## LGcode Go

LGcode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af LGcode-teamet, som er testet og verificeret til at fungere godt med LGcode.

1. Kør kommandoen `/connect` i TUI, vælg `LGcode Go`, og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Sådan bruger du Amazon Bedrock med LGcode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører lgcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `lgcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i LGcode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere lgcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at lgcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem lgcode-konfigurationen.
...
gemme tokenet i lgcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
LGcode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse LGcode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`lgcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `lgcode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med LGcode:
...
Sådan bruger du Google Vertex AI med LGcode:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem lgcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) plugin for automatisk at logge hver LGcode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver LGcode-samtale opført som en separat session.
...
Du kan konfigurere lgcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere lgcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere lgcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for LGcode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/lgcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med LGcode:
...
3. Kopier API-nøglen til brug i LGcode.
...
6. **Vigtigt**: Før du bruger skymodeller i LGcode, skal du hente modelinformationen lokalt:
...
LGcode Zen er en liste over testede og verificerede modeller leveret af LGcode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>LGcode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **LGcode Zen**.

   ```txt
   /connect
   ```

3. Indtast LGcode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem lgcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem lgcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LGcode:
...
Du kan også tilpasse modeller gennem lgcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med lgcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `lgcode.json`-filen i projektkataloget:
...
`limit`-felterne lader LGcode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `lgcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek lgcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i lgcode-konfigurationen.
