---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Loongcode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Loongcode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/loongcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Loongcode-konfigurationen.
...
Loongcode Zen er en liste over modeller leveret af Loongcode-teamet, som er blevet testet og verificeret til at fungere godt med Loongcode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Loongcode Zen.
:::
...
Det fungerer som alle andre udbydere i Loongcode og er helt valgfrit at bruge.

---

## Loongcode Go

Loongcode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Loongcode-teamet, som er testet og verificeret til at fungere godt med Loongcode.

1. Kør kommandoen `/connect` i TUI, vælg `Loongcode Go`, og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Sådan bruger du Amazon Bedrock med Loongcode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører loongcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `loongcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Loongcode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere loongcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at loongcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem loongcode-konfigurationen.
...
gemme tokenet i loongcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Loongcode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Loongcode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`loongcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Loongcode:
...
Sådan bruger du Google Vertex AI med Loongcode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører loongcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem loongcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) plugin for automatisk at logge hver Loongcode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Loongcode-samtale opført som en separat session.
...
Du kan konfigurere loongcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere loongcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere loongcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Loongcode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/loongcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Loongcode:
...
3. Kopier API-nøglen til brug i Loongcode.
...
6. **Vigtigt**: Før du bruger skymodeller i Loongcode, skal du hente modelinformationen lokalt:
...
Loongcode Zen er en liste over testede og verificerede modeller leveret af Loongcode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Loongcode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Loongcode Zen**.

   ```txt
   /connect
   ```

3. Indtast Loongcode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem loongcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem loongcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Loongcode:
...
Du kan også tilpasse modeller gennem loongcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med loongcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `loongcode.json`-filen i projektkataloget:
...
`limit`-felterne lader Loongcode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `loongcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek loongcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i loongcode-konfigurationen.
