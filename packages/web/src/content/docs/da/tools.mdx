...
Her er alle de innebygde verktøyene som er tilgængelige i Loongcode.
...
:::note
Dette verktøyet er bare tilgængelig når `LOONGCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `LOONGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Loongcode-leverandøren eller når miljøvariabelen `LOONGCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
