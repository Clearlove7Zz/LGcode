...
Her er alle de innebygde verktøyene som er tilgængelige i LGcode.
...
:::note
Dette verktøyet er bare tilgængelig når `LGCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `LGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger LGcode-leverandøren eller når miljøvariabelen `LGCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
