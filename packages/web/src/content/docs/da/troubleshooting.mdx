...
For at fejlfinde problemer med LGcode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\lgcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `lgcode --log-level DEBUG`.

---

## Lagring

lgcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\lgcode`
...
LGcode Desktop kører en lokal LGcode-server (`lgcode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `LGcode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (eller `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\lgcode\lgcode.jsonc`
...
LGcode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\lgcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så LGcode kan genopbygge den.

1. Afslut LGcode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/lgcode`
- **Linux**: slet `~/.cache/lgcode` (eller kør `rm -rf ~/.cache/lgcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\lgcode`

3. Start LGcode Desktop på ny.

---

### Løs problemer med serverforbindelse

LGcode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `lgcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LGCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LGCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver LGcode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i LGcode.

---

### Meddelelser vises ikke

LGcode Desktop viser kun systemvarsler når:

- varsler er aktiveret for LGcode i dine OS-indstillinger, og
...
1. Afslut LGcode Desktop.
2. Find og slet disse filer (de findes i LGcode Desktop-appens datamappe):

- `lgcode.settings.dat` (skrivebordsstandardserver URL)
- `lgcode.global.dat` og `lgcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med LGcode:
...
For at finde ud af hvilke modeller du har adgang til, kør `lgcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\lgcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. lgcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\lgcode`

2. Start lgcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge lgcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
lgcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
