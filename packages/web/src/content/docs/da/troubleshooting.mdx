...
For at fejlfinde problemer med Loongcode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\loongcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `loongcode --log-level DEBUG`.

---

## Lagring

loongcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/loongcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\loongcode`
...
Loongcode Desktop kører en lokal Loongcode-server (`loongcode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Loongcode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/loongcode/loongcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\loongcode\loongcode.jsonc`
...
Loongcode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/loongcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\loongcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Loongcode kan genopbygge den.

1. Afslut Loongcode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/loongcode`
- **Linux**: slet `~/.cache/loongcode` (eller kør `rm -rf ~/.cache/loongcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\loongcode`

3. Start Loongcode Desktop på ny.

---

### Løs problemer med serverforbindelse

Loongcode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `loongcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LOONGCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LOONGCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Loongcode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Loongcode.

---

### Meddelelser vises ikke

Loongcode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Loongcode i dine OS-indstillinger, og
...
1. Afslut Loongcode Desktop.
2. Find og slet disse filer (de findes i Loongcode Desktop-appens datamappe):

- `loongcode.settings.dat` (skrivebordsstandardserver URL)
- `loongcode.global.dat` og `loongcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Loongcode:
...
For at finde ud af hvilke modeller du har adgang til, kør `loongcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\loongcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. loongcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\loongcode`

2. Start loongcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge loongcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
loongcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
