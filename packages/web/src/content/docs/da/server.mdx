---
title: Server
description: Interager med lgcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `lgcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en lgcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `lgcode.local` |
...
Indstil `LGCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `lgcode`, eller indstil `LGCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `lgcode serve` og `lgcode web`.
...
Når du kører `lgcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug lgcode-serveren til at interagere med lgcode programmatisk.
:::

Denne arkitektur lader lgcode understøtte flere klienter og lader dig interagere med lgcode programmatisk.

Du kan køre `lgcode serve` for at starte en selvstændig server. Hvis du har
lgcode TUI kørende, vil `lgcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af LGcode [IDE](/docs/ide) plugins.
...
LGcode-serveren viser følgende API'er.
