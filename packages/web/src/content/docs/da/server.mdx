...
Indstil `LGCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `opencode`, eller indstil `LGCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `lgcode serve` og `lgcode web`.
...
:::tip
Brug lgcode-serveren til at interagere med opencode programmatisk.
:::
...
opencode TUI kørende, vil `lgcode serve` starte en ny server.
