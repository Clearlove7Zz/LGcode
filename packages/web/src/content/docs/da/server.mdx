---
title: Server
description: Interager med loongcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `loongcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en loongcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `loongcode.local` |
...
Indstil `LOONGCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `loongcode`, eller indstil `LOONGCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `loongcode serve` og `loongcode web`.
...
Når du kører `loongcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug loongcode-serveren til at interagere med loongcode programmatisk.
:::

Denne arkitektur lader loongcode understøtte flere klienter og lader dig interagere med loongcode programmatisk.

Du kan køre `loongcode serve` for at starte en selvstændig server. Hvis du har
loongcode TUI kørende, vil `loongcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Loongcode [IDE](/docs/ide) plugins.
...
Loongcode-serveren viser følgende API'er.
