---
title: TUI
description: Using the Loongcode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Loongcode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Loongcode starts the TUI for the current directory.
...
When using the Loongcode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Loongcode. Allows you to select from available providers and add their API keys.
...
Exit Loongcode. _Aliases_: `/quit`, `/q`
...
This is separate from `loongcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Loongcode waits after the leader key. Defaults to `2000`.
...
Use `LOONGCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `loongcode.default`.
