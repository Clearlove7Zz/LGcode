---
title: TUI
description: Using the LGcode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LGcode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running LGcode starts the TUI for the current directory.
...
When using the LGcode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to LGcode. Allows you to select from available providers and add their API keys.
...
This is separate from `lgcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long LGcode waits after the leader key. Defaults to `2000`.
...
Use `LGCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `lgcode.default`.
