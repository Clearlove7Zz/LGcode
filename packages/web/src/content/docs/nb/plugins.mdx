...
description: Skriv dine egne programtillegg for å utvide Loongcode.
---

Plugins lar deg utvide Loongcode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Loongcode.
...
- `.loongcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/loongcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/loongcode/loongcode.json`)
2. Prosjektkonfigurasjon (`loongcode.json`)
3. Global plugin-katalog (`~/.config/loongcode/plugins/`)
4. Prosjektpluginkatalog (`.loongcode/plugins/`)
...
Loongcode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Loongcode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Loongcode.
...
:::note
Hvis du bruker Loongcode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Loongcode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Loongcode:
...
`tool`-hjelperen lager et tilpasset verktøy som Loongcode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Loongcode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://modelhub.lgdg.cc/docs/sdk) for detaljer.
