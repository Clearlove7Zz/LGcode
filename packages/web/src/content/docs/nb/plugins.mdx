...
description: Skriv dine egne programtillegg for å utvide LGcode.
---

Plugins lar deg utvide LGcode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til LGcode.
...
- `.lgcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/lgcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/lgcode/lgcode.json`)
2. Prosjektkonfigurasjon (`lgcode.json`)
3. Global plugin-katalog (`~/.config/lgcode/plugins/`)
4. Prosjektpluginkatalog (`.lgcode/plugins/`)
...
LGcode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En LGcode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide LGcode.
...
:::note
Hvis du bruker LGcode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre LGcode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til LGcode:
...
`tool`-hjelperen lager et tilpasset verktøy som LGcode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for LGcode sammen med innebygde verktøy.
