...
Opprett `.lgcode/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom LGcode-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i LGcode [config](/docs/config):
...
LGcode inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
