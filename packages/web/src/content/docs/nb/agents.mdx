...
Det er to typer agenter i Loongcode; primære agenter og underagenter.
...
Loongcode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Loongcode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Loongcode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `loongcode.json`:
...
Hvis ingen temperatur er spesifisert, bruker Loongcode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Loongcode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Loongcode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Loongcode Zen](/docs/zen), vil du bruke `loongcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `loongcode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/Clearlove7Zz/LoongCode).
:::
