---
title: Tjener
description: Samhandle med Loongcode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `loongcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Loongcode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `loongcode.local` |
...
Sett `LOONGCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `loongcode` som standard, eller sett `LOONGCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `loongcode serve` og `loongcode web`.
...
Når du kjører `loongcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Loongcode-serveren til å samhandle med Loongcode programmatisk.
:::

Denne arkitekturen lar Loongcode støtte flere klienter og lar deg samhandle med Loongcode programmatisk.

Du kan kjøre `loongcode serve` for å starte en frittstående server. Hvis du har
Loongcode TUI kjører, vil `loongcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Loongcode [IDE](/docs/ide) plugins.
...
loongcode-serveren viser følgende APIer.
