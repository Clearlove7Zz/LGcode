---
title: Tjener
description: Samhandle med LGcode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `lgcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en LGcode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `lgcode.local` |
...
Sett `LGCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `lgcode` som standard, eller sett `LGCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `lgcode serve` og `lgcode web`.
...
Når du kjører `lgcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk LGcode-serveren til å samhandle med LGcode programmatisk.
:::

Denne arkitekturen lar LGcode støtte flere klienter og lar deg samhandle med LGcode programmatisk.

Du kan kjøre `lgcode serve` for å starte en frittstående server. Hvis du har
LGcode TUI kjører, vil `lgcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av LGcode [IDE](/docs/ide) plugins.
...
lgcode-serveren viser følgende APIer.
