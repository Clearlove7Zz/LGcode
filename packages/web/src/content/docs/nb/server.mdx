...
Sett `LGCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `LGCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `lgcode serve` og `lgcode web`.
