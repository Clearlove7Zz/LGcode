---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i LGcode.
---

import config from "../../../../config.mjs"
export const console = config.console

LGcode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i LGcode-konfigurasjonen.
...
i `~/.local/share/lgcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i LGcode
konfig.
...
LGcode Zen er en liste over modeller levert av LGcode-teamet som har vært
testet og verifisert for å fungere godt med LGcode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med LGcode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `LGcode Zen` og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Det fungerer som alle andre leverandører i LGcode og er helt valgfritt å bruke.

---

## LGcode Go

LGcode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av LGcode-teamet som har vært
testet og verifisert for å fungere godt med LGcode.

1. Kjør kommandoen `/connect` i TUI, velg `LGcode Go`, og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Slik bruker du Amazon Bedrock med LGcode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører LGcode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `lgcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i lgcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere lgcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at LGcode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom LGcode-konfigurasjonen.
...
for å lagre token i LGcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
LGcode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse LGcode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`lgcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [lgcode-gitlab-auth](https://www.npmjs.com/package/lgcode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `lgcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med LGcode:
...
Slik bruker du Google Vertex AI med LGcode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører LGcode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom LGcode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [lgcode-helicone-session](https://github.com/H2Shami/lgcode-helicone-session) plugin for automatisk å logge hver LGcode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver LGcode-samtale oppført som en separat økt.
...
Du kan konfigurere LGcode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere LGcode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere LGcode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for LGcode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/lgcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med LGcode:
...
3. Kopier API-nøkkelen for bruk i LGcode.
...
6. **Viktig**: Før du bruker skymodeller i LGcode, må du hente modellinformasjonen lokalt:
...
LGcode Zen er en liste over testede og verifiserte modeller levert av LGcode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>LGcode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **LGcode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn LGcode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom LGcode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom LGcode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LGcode:
...
Du kan også tilpasse modeller gjennom LGcode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med LGcode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `lgcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar LGcode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `lgcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk LGcode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i LGcode-konfigurasjonen.
