---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Loongcode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Loongcode-konfigurasjonen.
...
i `~/.local/share/loongcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Loongcode
konfig.
...
Loongcode Zen er en liste over modeller levert av Loongcode-teamet som har vært
testet og verifisert for å fungere godt med Loongcode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Loongcode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Loongcode Zen` og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Det fungerer som alle andre leverandører i Loongcode og er helt valgfritt å bruke.

---

## Loongcode Go

Loongcode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Loongcode-teamet som har vært
testet og verifisert for å fungere godt med Loongcode.

1. Kjør kommandoen `/connect` i TUI, velg `Loongcode Go`, og gå til [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Slik bruker du Amazon Bedrock med Loongcode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Loongcode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `loongcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i loongcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere loongcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Loongcode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Loongcode-konfigurasjonen.
...
for å lagre token i Loongcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Loongcode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Loongcode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`loongcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `loongcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Loongcode:
...
Slik bruker du Google Vertex AI med Loongcode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Loongcode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Loongcode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) plugin for automatisk å logge hver Loongcode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Loongcode-samtale oppført som en separat økt.
...
Du kan konfigurere Loongcode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Loongcode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Loongcode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Loongcode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/loongcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Loongcode:
...
3. Kopier API-nøkkelen for bruk i Loongcode.
...
6. **Viktig**: Før du bruker skymodeller i Loongcode, må du hente modellinformasjonen lokalt:
...
Loongcode Zen er en liste over testede og verifiserte modeller levert av Loongcode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Loongcode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Loongcode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Loongcode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Loongcode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Loongcode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Loongcode:
...
Du kan også tilpasse modeller gjennom Loongcode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Loongcode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `loongcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Loongcode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `loongcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Loongcode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Loongcode-konfigurasjonen.
