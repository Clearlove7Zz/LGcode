...
Du kan legge til eksterne verktøy i LGcode ved å bruke _Model Context Protocol_ eller MCP. LGcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [LGcode Config](https://modelhub.lgdg.cc/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="lgcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/lgcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
LGcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil LGcode:
...
Hvis serveren krever autentisering, vil LGcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `lgcode mcp auth <server-name>`.
...
```bash
lgcode mcp auth my-oauth-server
```
...
```bash
lgcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil LGcode lagre tokens sikkert i `~/.local/share/lgcode/mcp-auth.json`.
...
lgcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i LGcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom LGcode-konfigurasjonen som alle andre verktøy.
...
```json title="lgcode.json" {11, 14-18}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
lgcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble LGcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver LGcode-samtale oppført som en separat økt.
