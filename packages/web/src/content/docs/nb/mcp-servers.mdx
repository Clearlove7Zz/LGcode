...
Du kan legge til eksterne verktøy i Loongcode ved å bruke _Model Context Protocol_ eller MCP. Loongcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Loongcode Config](https://modelhub.lgdg.cc/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="loongcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/loongcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="loongcode.jsonc" {15}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Loongcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Loongcode:
...
Hvis serveren krever autentisering, vil Loongcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `loongcode mcp auth <server-name>`.
...
```json title="loongcode.json" {7-11}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
loongcode mcp auth my-oauth-server
```
...
```bash
loongcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Loongcode lagre tokens sikkert i `~/.local/share/loongcode/mcp-auth.json`.
...
loongcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Loongcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Loongcode-konfigurasjonen som alle andre verktøy.
...
```json title="loongcode.json" {11, 14-18}
{
  "$schema": "https://modelhub.lgdg.cc/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
loongcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Loongcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Loongcode-samtale oppført som en separat økt.
