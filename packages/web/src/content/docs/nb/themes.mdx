...
Med LGcode kan du velge fra ett av flere innebygde temaer, bruke et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruker LGcode vårt eget `lgcode`-tema.
...
LGcode støtter et fleksibelt JSON-basert temasystem som lar brukere enkelt lage og tilpasse temaer.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/lgcode/themes/*.json` eller `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.lgcode/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.lgcode/themes/*.json`
