...
Agentferdigheter lar Loongcode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
loongcode søker etter disse stedene:

- Prosjektkonfigurasjon: `.loongcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/loongcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går loongcode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.loongcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.loongcode/skills/git-release/SKILL.md` slik:
...
loongcode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `loongcode.json`:
...
**For innebygde agenter** (i `loongcode.json`):
