...
description: Cree herramientas que LLM pueda llamar en loongcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de loongcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.loongcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/loongcode/tools/`.
...
```ts title=".loongcode/tools/database.ts" {1}
import { tool } from "@loongcode-ai/plugin"
...
```ts title=".loongcode/tools/math.ts"
import { tool } from "@loongcode-ai/plugin"
...
```ts title=".loongcode/tools/bash.ts"
import { tool } from "@loongcode-ai/plugin"
...
```ts title=".loongcode/tools/project.ts" {8}
import { tool } from "@loongcode-ai/plugin"
...
```python title=".loongcode/tools/add.py"
import sys
...
```ts title=".loongcode/tools/python-add.ts" {10}
import { tool } from "@loongcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".loongcode/tools/add.py")
