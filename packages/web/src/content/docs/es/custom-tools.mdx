...
description: Cree herramientas que LLM pueda llamar en lgcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de lgcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.lgcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/lgcode/tools/`.
...
```ts title=".lgcode/tools/database.ts" {1}
import { tool } from "@lgcode-ai/plugin"
...
```ts title=".lgcode/tools/project.ts" {8}
import { tool } from "@lgcode-ai/plugin"
...
```python title=".lgcode/tools/add.py"
import sys
...
```ts title=".lgcode/tools/python-add.ts" {10}
import { tool } from "@lgcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".lgcode/tools/add.py")
