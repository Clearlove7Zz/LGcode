...
Con LGcode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, LGcode usa nuestro propio tema `lgcode`.
...
LGcode viene con varios temas integrados.
...
- Quiere que LGcode coincida con la apariencia de su terminal
...
LGcode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/lgcode/themes/*.json` o `$XDG_CONFIG_HOME/lgcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.lgcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.lgcode/themes/*.json`
