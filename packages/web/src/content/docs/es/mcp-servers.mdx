...
Puede agregar herramientas externas a Loongcode utilizando el _Model Context Protocol_ o MCP. Loongcode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Loongcode Config](https://modelhub.lgdg.cc/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/loongcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Loongcode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Loongcode:
...
Si el servidor requiere autenticación, Loongcode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `loongcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Loongcode almacenará los tokens de forma segura en `~/.local/share/loongcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Loongcode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Loongcode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Loongcode a su cuenta Sentry.
