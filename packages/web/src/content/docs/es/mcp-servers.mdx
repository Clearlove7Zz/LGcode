...
Puede agregar herramientas externas a LGcode utilizando el _Model Context Protocol_ o MCP. LGcode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [LGcode Config](https://modelhub.lgdg.cc/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/lgcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
LGcode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, LGcode:
...
Si el servidor requiere autenticación, LGcode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `lgcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, LGcode almacenará los tokens de forma segura en `~/.local/share/lgcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en LGcode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración LGcode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar LGcode a su cuenta Sentry.
