---
title: Proveedores
description: Usando cualquier proveedor LLM en Loongcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Loongcode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Loongcode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Loongcode
configuración.
...
Loongcode Zen es una lista de modelos proporcionados por el equipo Loongcode que han sido
probado y verificado para funcionar bien con Loongcode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Loongcode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione loongcode y diríjase a [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/auth).
...
Funciona como cualquier otro proveedor en Loongcode y su uso es completamente opcional.

---

## Loongcode Go

Loongcode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Loongcode que han sido
probado y verificado para funcionar bien con Loongcode.

1. Ejecute el comando `/connect` en TUI, seleccione `Loongcode Go` y diríjase a [modelhub.lgdg.cc/auth](https://modelhub.lgdg.cc/zen).
...
Para usar Amazon Bedrock con Loongcode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta loongcode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `loongcode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Loongcode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar loongcode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que loongcode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración loongcode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Loongcode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación loongcode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Loongcode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Loongcode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `loongcode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `loongcode.json`:
...
Para utilizar su suscripción GitHub Copilot con loongcode:
...
Para utilizar Google Vertex AI con Loongcode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta loongcode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de loongcode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [loongcode-helicone-session](https://github.com/H2Shami/loongcode-helicone-session) para registrar automáticamente cada conversación Loongcode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Loongcode enumerada como una sesión separada.
...
Puede configurar loongcode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar loongcode para usar modelos locales a través de LM Studio.
...
Puedes configurar loongcode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Loongcode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/loongcode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Loongcode:
...
3. Copie la clave API para usarla en Loongcode.
...
6. **Importante**: Antes de usar modelos en la nube en Loongcode, debe obtener la información del modelo localmente:
...
Loongcode Zen es una lista de modelos probados y verificados proporcionada por el equipo Loongcode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Loongcode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Loongcode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Loongcode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración loongcode.
...
5. También puedes personalizarlos a través de tu configuración loongcode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Loongcode:
...
También puede personalizar modelos a través de su configuración loongcode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con loongcode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `loongcode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Loongcode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `loongcode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración loongcode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración loongcode.
