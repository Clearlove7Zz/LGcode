...
Para depurar problemas con LGcode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/lgcode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\lgcode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `lgcode --log-level DEBUG`.

---

## Almacenamiento

lgcode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/lgcode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\lgcode`
...
LGcode Desktop ejecuta un servidor LGcode local (el sidecar `lgcode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `LGcode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/lgcode/lgcode.jsonc` (o `~/.config/lgcode/lgcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/lgcode/lgcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\lgcode\lgcode.jsonc`
...
LGcode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/lgcode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\lgcode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.lgcode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que LGcode pueda reconstruirlo.

1. Salga de LGcode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/lgcode`
- **Linux**: eliminar `~/.cache/lgcode` (o ejecutar `rm -rf ~/.cache/lgcode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\lgcode`

3. Reinicie el escritorio LGcode.

---

### Solucionar problemas de conexión del servidor

LGcode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `lgcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LGCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LGCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio LGcode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de LGcode.

---

### Notificaciones que no se muestran

LGcode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para LGcode en la configuración de su sistema operativo, y
...
1. Salga del escritorio LGcode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio LGcode):

- `lgcode.settings.dat` (URL del servidor predeterminado de escritorio)
- `lgcode.global.dat` y `lgcode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `lgcode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `lgcode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/lgcode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\lgcode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. lgcode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\lgcode`

2. Reinicie lgcode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a lgcode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
lgcode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
