...
Para depurar problemas con Loongcode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\loongcode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `loongcode --log-level DEBUG`.

---

## Almacenamiento

loongcode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/loongcode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\loongcode`
...
Loongcode Desktop ejecuta un servidor Loongcode local (el sidecar `loongcode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Loongcode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/loongcode/loongcode.jsonc` (o `~/.config/loongcode/loongcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/loongcode/loongcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\loongcode\loongcode.jsonc`
...
Loongcode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/loongcode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\loongcode\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Loongcode pueda reconstruirlo.

1. Salga de Loongcode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/loongcode`
- **Linux**: eliminar `~/.cache/loongcode` (o ejecutar `rm -rf ~/.cache/loongcode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\loongcode`

3. Reinicie el escritorio Loongcode.

---

### Solucionar problemas de conexión del servidor

Loongcode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `loongcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LOONGCODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LOONGCODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Loongcode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Loongcode.

---

### Notificaciones que no se muestran

Loongcode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Loongcode en la configuración de su sistema operativo, y
...
1. Salga del escritorio Loongcode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Loongcode):

- `loongcode.settings.dat` (URL del servidor predeterminado de escritorio)
- `loongcode.global.dat` y `loongcode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Loongcode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/Clearlove7Zz/LoongCode/issues**](https://github.com/Clearlove7Zz/LoongCode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**modelhub.lgdg.cc/discord**](https://modelhub.lgdg.cc/discord)
...
3. Asegúrese de tener la última versión con `loongcode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `loongcode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/loongcode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\loongcode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. loongcode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\loongcode`

2. Reinicie loongcode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a loongcode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
loongcode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
