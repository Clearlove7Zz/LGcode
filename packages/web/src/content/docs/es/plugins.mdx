---
title: Complementos
description: Escriba sus propios complementos para extender LGcode.
---

Los complementos le permiten extender LGcode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de LGcode.
...
- `.lgcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/lgcode/plugins/` - Complementos globales
...
```json title="lgcode.json"
{
  ...
  "plugin": ["lgcode-helicone-session", "lgcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/lgcode/node_modules/`.
...
1. Configuración global (`~/.config/lgcode/lgcode.json`)
2. Configuración del proyecto (`lgcode.json`)
3. Directorio global de complementos (`~/.config/lgcode/plugins/`)
4. Directorio de complementos del proyecto (`.lgcode/plugins/`)
...
```json title=".lgcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
LGcode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".lgcode/plugins/my-plugin.ts"
...
```js title=".lgcode/plugins/example.js"
...
- `client`: Un cliente SDK lgcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar lgcode.
...
```js title=".lgcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "lgcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio LGcode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que lgcode lea archivos `.env`:

```javascript title=".lgcode/plugins/env-protection.js"
...
```javascript title=".lgcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a lgcode:

```ts title=".lgcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@lgcode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que lgcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para lgcode junto con las herramientas integradas.
...
```ts title=".lgcode/plugins/compaction.ts"
import type { Plugin } from "@lgcode-ai/plugin"
...
```ts title=".lgcode/plugins/custom-compaction.ts"
import type { Plugin } from "@lgcode-ai/plugin"
