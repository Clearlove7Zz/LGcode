...
Las habilidades del agente permiten a Loongcode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Loongcode busca estas ubicaciones:

- Configuración del proyecto: `.loongcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/loongcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Loongcode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.loongcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/loongcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.loongcode/skills/git-release/SKILL.md` así:
...
Loongcode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `loongcode.json`:
...
**Para agentes integrados** (en `loongcode.json`):
