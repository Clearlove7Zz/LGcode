...
Las habilidades del agente permiten a LGcode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
LGcode busca estas ubicaciones:

- Configuración del proyecto: `.lgcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/lgcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, LGcode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.lgcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/lgcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.lgcode/skills/git-release/SKILL.md` así:
...
LGcode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `lgcode.json`:
...
**Para agentes integrados** (en `lgcode.json`):
