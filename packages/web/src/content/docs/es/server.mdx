...
El comando `lgcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opencode puede usar.
...
Configure `LGCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `opencode`, o configure `LGCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `lgcode serve` como a `lgcode web`.
...
opencode TUI ejecutándose, `lgcode serve` iniciará un nuevo servidor.
