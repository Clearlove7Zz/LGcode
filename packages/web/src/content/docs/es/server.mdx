---
title: Servidor
description: Interactuar con el servidor loongcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `loongcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente loongcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `loongcode.local` |
...
Configure `LOONGCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `loongcode`, o configure `LOONGCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `loongcode serve` como a `loongcode web`.
...
Cuando ejecuta `loongcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor loongcode para interactuar con loongcode mediante programación.
:::

Esta arquitectura permite que loongcode admita múltiples clientes y le permite interactuar con loongcode mediante programación.

Puede ejecutar `loongcode serve` para iniciar un servidor independiente. Si tienes el
loongcode TUI ejecutándose, `loongcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Loongcode [IDE](/docs/ide).
...
El servidor loongcode expone las siguientes API.
