---
title: Servidor
description: Interactuar con el servidor lgcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `lgcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente lgcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `lgcode.local` |
...
Configure `LGCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `lgcode`, o configure `LGCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `lgcode serve` como a `lgcode web`.
...
Cuando ejecuta `lgcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor lgcode para interactuar con lgcode mediante programación.
:::

Esta arquitectura permite que lgcode admita múltiples clientes y le permite interactuar con lgcode mediante programación.

Puede ejecutar `lgcode serve` para iniciar un servidor independiente. Si tienes el
lgcode TUI ejecutándose, `lgcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos LGcode [IDE](/docs/ide).
...
El servidor lgcode expone las siguientes API.
