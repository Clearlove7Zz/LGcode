...
Las herramientas permiten que LLM realice acciones en su código base. LGcode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en LGcode.
...
:::note
Esta herramienta solo está disponible cuando `LGCODE_EXPERIMENTAL_LSP_TOOL=true` (o `LGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor LGcode o cuando la variable de entorno `LGCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar LGcode:

```bash
LGCODE_ENABLE_EXA=1 opencode
```
