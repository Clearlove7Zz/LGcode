...
Las herramientas permiten que LLM realice acciones en su código base. Loongcode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Loongcode.
...
:::note
Esta herramienta solo está disponible cuando `LOONGCODE_EXPERIMENTAL_LSP_TOOL=true` (o `LOONGCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Loongcode o cuando la variable de entorno `LOONGCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Loongcode:

```bash
LOONGCODE_ENABLE_EXA=1 loongcode
```
