...
This document covers meaningful contract changes introduced on the `feat/loongcode-embedded-api` branch since its divergence from `origin/dev`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Canonical tagged tool file sources in `@loongcode-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under Loongcode's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `LOONGCODE_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
