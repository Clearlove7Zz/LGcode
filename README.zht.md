...
# 套件管理員
npm i -g loongcode-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install loongcode             # Windows
choco install loongcode             # Windows
brew install Clearlove7Zz/tap/loongcode # macOS 與 Linux（推薦，始終保持最新）
brew install loongcode              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S loongcode            # Arch Linux (Stable)
paru -S loongcode-bin               # Arch Linux (Latest from AUR)
mise use -g loongcode               # 任何作業系統
nix run nixpkgs#loongcode           # 或使用 github:Clearlove7Zz/LoongCode 以取得最新開發分支
```
...
Loongcode 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/Clearlove7Zz/LoongCode/releases) 或 [modelhub.lgdg.cc/download](https://modelhub.lgdg.cc/download) 下載。
...
1. `$LOONGCODE_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.loongcode/bin` - 預設備用路徑
...
Loongcode 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，Loongcode 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://modelhub.lgdg.cc/docs/agents) 的資訊。

### 線上文件

關於如何設定 Loongcode 的詳細資訊，請參閱我們的 [**官方文件**](https://modelhub.lgdg.cc/docs)。

### 參與貢獻

如果您有興趣參與 Loongcode 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Loongcode 進行開發

如果您正在開發與 Loongcode 相關的專案，並在名稱中使用了 "loongcode"（例如 "loongcode-dashboard" 或 "loongcode-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Loongcode 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=738j8655-cd59-4633-a30a-1124e0096789&qr_code=true) | [X.com](https://x.com/loongcode)
